...
#ifdef ODM_WT_EDIT
//Shouxing.Tang@ODM_WT.MM.AudioDriver.Machine 2020/04/16, fix dial pop noise from headphone
#define ANALOG_HPTRIM_FOR_CUST
#else
//#define ANALOG_HPTRIM_FOR_CUST
#endif
#ifdef ODM_WT_EDIT
/*Shouxing.Tang@ODM_WT.MM.AudioDriver.Machine 2020/05/11, disable hp imped detect*/
#define BYPASS_HPIMP
#endif
	...
	if (enable) {
		if (GetAdcStatus() == false) {
			...
			if (mCodec_data->mAudio_Ana_Mux
				...
				/* Enable MICBIAS0, MISBIAS0 = 1P9V */
#ifdef ODM_WT_EDIT
				//Shouxing.Tang@ODM_WT.MM.Audiodriver.codec, 2020/4/4, change mic bias0 2.7v for 206A1
				Ana_Set_Reg(AUDENC_ANA_CON8, 0x0071, 0xffff);
#else
				Ana_Set_Reg(AUDENC_ANA_CON8, 0x0021, 0xffff);
#endif
