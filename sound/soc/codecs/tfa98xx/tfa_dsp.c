...
#ifdef VENDOR_EDIT
/* Jun.Liang@MM.AudioDriver.Codec, 2019/10/24, add for dual spk */
...
void tfa_getBoundary_v6(int slave_address, int* min_mohms, int* max_mohms)
{
	...
	} else if ((project_id == 19354) || (project_id == 19358) || (project_id == 19359)) {
		*min_mohms = 5500;
		*max_mohms = 8500;
	} else {
		*min_mohms = 5000;
...
/*xiang.fei@PSW.MM.AudioDriver.Codec, 2018/03/12, Add for speaker resistance*/
enum Tfa98xx_Error tfaRunSpeakerCalibration_result(struct tfa_device *tfa, int *result)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int calibrateDone;
	int spkr_count = 0;
...
enum Tfa98xx_Error tfa_dev_set_state(struct tfa_device *tfa, enum tfa_state state)
{
	...
	case TFA_STATE_INIT_CF:      /* coolflux HW access possible (~initcf) */
		...
		if (!tfa->is_probus_device)
		{
		    #ifdef VENDOR_EDIT
		    /*xiang.fei@PSW.MM.AudioDriver.Codec, 2018/03/12, Add for speaker resistance*/
		    err = tfa98xx_set_mtp(tfa, 1, TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_MSK);
		    if (err != tfa_error_ok) {
			    return err;
		    }
		    #endif /* VENDOR_EDIT */
