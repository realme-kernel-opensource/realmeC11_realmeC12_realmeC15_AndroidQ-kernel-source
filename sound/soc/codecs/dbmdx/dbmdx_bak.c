...
out:
	p->unlock(p);
	set_vaud18_enable(false);
	udelay(100);
	set_vaud18_enable(true);
	return ret;
}
					...
					bool to_start_buffering)
{
...
#endif
			/*Aispeech didn't define this parameter,so set to the default value 0.*/
#if 0
	...
	/*Aispeech didn't define this parameter "SCORE".*/
#if 0
	...
	do {
		...
		if (nr_samples) {
			...
			if ((nr_samples == nr_samples_in_fw) &&
				(DBMDX_MSLEEP_IF_AUDIO_BUFFER_EMPTY != 0))
...
static int dbmdx_platform_probe(struct platform_device *pdev)
{
	...
	if (clk_cnt == 0) {
		pr_info("%s: DBMDX enable rtc32k once!\n", __func__);
		ret = mt6359_upmu_set_rg_rtc32k_1v8_1_pdn(0x0);
		pr_info("%s: DBMDX enable rtc32k once ret = %d\n", __func__, ret);
		clk_cnt++;
	}
