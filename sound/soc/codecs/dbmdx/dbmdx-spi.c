...
static struct mt_chip_conf cf_spi_conf =
{
		.setuptime = 15,
        .holdtime = 15,
        .high_time = 30, // 10--6m 15--4m 20--3m 30--2m [ 60--1m 120--0.5m  300--0.2m]
        .low_time  = 30,
        .cs_idletime = 30,
        .ulthgh_thrsh = 0,
        .cpol = 0,
...
#else
static const struct mtk_chip_config cf_spi_conf = {
    .rx_mlsb = 1,
    .tx_mlsb = 1,
    .cs_pol = 0,
};
...
int spi_set_speed(struct dbmdx_private *p, int index)
{
    ...
    spi->controller_data = (void*)&cf_spi_conf;
	if (spi->max_speed_hz != spi_rate || spi->mode != spi_mode) {
		/*For MTK6763 platform,it uses &cf_spi_conf to configure the spi speed */
		/*As Agold used D4 EVB for test, it's anable to support High speed (4800000Hz), or will get failure during load acoustic model */ 
		/*it is reference code here.Customer need to check by themselves how it configured.*/
		/*For Agold,the spi speed = 106 MHz/(55+55)==963636hz */
#ifndef CONFIG_SPI_MT65XX
		if(spi_rate == 4800000)
		{
			cf_spi_conf.high_time = 40;
			cf_spi_conf.low_time = 40;
			cf_spi_conf.com_mod = DMA_TRANSFER;
		}
		else //960000 HZ  
		{
			cf_spi_conf.high_time = 55;
			cf_spi_conf.low_time = 55;
			cf_spi_conf.com_mod = FIFO_TRANSFER;
		}
#endif
