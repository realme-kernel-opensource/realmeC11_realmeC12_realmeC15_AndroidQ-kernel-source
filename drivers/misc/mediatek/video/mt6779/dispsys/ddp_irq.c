...
int disp_unregister_irq_callback(DDP_IRQ_CALLBACK cb)
{
	...
	if (i == DISP_MAX_IRQ_CALLBACK) {
		DDP_PR_ERR("Try to unregister callback function %p which was not registered\n",
			   cb);
		return -1;
	}
	return 0;
					...
					DDP_IRQ_CALLBACK cb)
{
	...
	if (i == DISP_MAX_IRQ_CALLBACK) {
		DDP_PR_ERR("try to unregister callback function but not registered. module=%d cb=%p\n",
			   module, cb);
		return -1;
	}
	return 0;
...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
	...
	if (irq == ddp_get_module_irq(DISP_MODULE_DSI0)) {
		...
		if (reg_val & (1 << 1))
			DDP_PR_ERR("MMSYS to MJC APB TX Error, MMSYS clock off but MJC clock on!\n");
