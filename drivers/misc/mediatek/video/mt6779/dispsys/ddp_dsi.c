...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/02/07, add for ramless aod */
#include "disp_lcm.h"
#endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/02/19, add for ramless aod */
		...
		struct LCM_DSI_PARAMS *dsi_params)
{
	...
	if (dsi_params->mode == SYNC_EVENT_VDO_MODE ||
		...
		#ifdef VENDOR_EDIT
		/* LiPing-m@PSW.MM.Display.LCD.Stable,2020-1-11, modify for video mode lcd fps */
...
/*
			chg_status = MIPITX_INREGBIT(DSI_PHY_REG[i]+
				MIPITX_PLL_CON1, FLD_RG_DSI_PLL_SDM_PCW_CHG);
...
int mipi_clk_change(enum DISP_MODULE_ENUM module, int en)
{
	...
	#ifdef VENDOR_EDIT
	/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/12/05, add for mipi clk change*/
			       ...
			       UINT8 *buffer, UINT8 buffer_size)
{
	...
	do {
		...
		if (ret == 0) {
			...
			DISP_PR_ERR("DSI Read Fail: dsi wait read ready timeout\n");
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/08, add for ramless write  aod area */
...
#ifdef VENDOR_EDIT
/*
 * Hao.Lin@PSW.MM.Display.LCD.Stability, 2019/12/18,
 * add for dynamic osc switch
 */

struct lcm_cmd {
	unsigned char cmd;
	unsigned char count;
	unsigned char para_list[10];
};


static struct lcm_cmd lcm_osc86m_setting[] = {
	{0xFC, 2, {0x5A,0x5A}},
	{0xB0, 1, {0x07}},
	{0xD7, 3, {0x43,0xBD,0x56}},
	{0xFE, 1, {0xB0}},
	{0xFE, 1, {0x30}},
	{0xFC, 2, {0xA5,0xA5}}
};

static struct lcm_cmd lcm_osc90m_setting[] = {
	{0xFC, 2, {0x5A,0x5A}},
	{0xB0, 1, {0x07}},
	{0xD7, 3, {0x33,0xBD,0xC3}},
	{0xFE, 1, {0xB0}},
	{0xFE, 1, {0x30}},
	{0xFC, 2, {0xA5,0xA5}}
};
...
static void lcm_write_cmd(struct lcm_cmd *table, unsigned int count, unsigned char force_update)
{
	struct cmdqRecStruct *handle = NULL;
	unsigned int i;
	...
	if (_is_power_on_status(DISP_MODULE_DSI0)) {
		...
		for (i = 0; i < count; i++) {
			unsigned cmd;
			cmd = table[i].cmd;
			DSI_set_cmdq_V2(DISP_MODULE_DSI0, handle, cmd, table[i].count, table[i].para_list, force_update);
		}
...
int dsi_osc_change(int msg, int en)
{
	int ret = 0;

	DISPFUNC();
	pr_info("[DISP] func | %s: en = %d\n", __func__, en);

	if (!strcmp(mtkfb_find_lcm_driver(), "oppo19551_samsung_ams644vk01_1080p_dsi_cmd")) {
		pr_debug("[DISP] osc find oppo19551_samsung_ams644vk01_1080p_dsi_cmd\n");
	} else {
		pr_err("[DISP] osc is not need\n");
		return ret;
	}

	if (en) {
		lcm_write_cmd(lcm_osc86m_setting, sizeof(lcm_osc86m_setting) / sizeof(struct lcm_cmd), 1);
	}else {
		lcm_write_cmd(lcm_osc90m_setting, sizeof(lcm_osc90m_setting) / sizeof(struct lcm_cmd), 1);
	}
...
int ddp_dsi_stop(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (DSI_REG[i]->DSI_MODE_CTRL.MODE == CMD_MODE) {
		...
		/* stop vdo mode */
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/02/14, add for ramless */
		if ((oppo_display_aod_ramless_support && rlta_enable)) {
...
static void push_cmd_table(void *cmdq, struct LCM_setting_table *table,
	unsigned int count, unsigned char force_update)
{
	...
	for (i = 0; i < count; i++) {
		...
		default:
			DSI_set_cmdq_V2_DSI0(cmdq,cmd,table[i].count,table[i].para_list,1);
		}
	}
}
	...
	void *params)
{
	...
	if (mode == 0) { /* V2C */
		...
		if(0) {
			poll_frame_done(cmdq_handle, i);
			push_cmd_table(cmdq_handle,lcm_normal_to_aod_setting,sizeof(lcm_normal_to_aod_setting) / sizeof(struct LCM_setting_table), 1);
		}
		/* 1. wait idle */
		DSI_POLLREG32(cmdq_handle, &DSI_REG[i]->DSI_INTSTA,
			0x80000000, 0x0);
		...
		push_cmd_table(cmdq_handle,lcm_aod_to_normal_setting,sizeof(lcm_aod_to_normal_setting) / sizeof(struct LCM_setting_table), 1);
		pr_info("[soso] c2v init code push finished\n");
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (return_val == lcm_esd_tb->para_list[0]) {
				...
				/* esd fail */
				DISP_PR_ERR("[DSI]cmp fail:read(0x%x)!=expect(0x%x)\n",
					    read_data0.byte1,
					    lcm_esd_tb->para_list[0]);
				ret = 1;
				break;
