...
#ifdef VENDOR_EDIT
/* Ling.Guo@PSW.MM.Display.LCD.Stability, 2019/01/21, add for mipi clk change */
...
extern bool oppo_display_aodlight_support;
extern bool oppo_display_aod_ramless_support;
extern bool oppo_display_tenbits_support;
extern bool oppo_display_elevenbits_support;
#endif /*VENDOR_EDIT*/
		       ...
		       unsigned long arg)
{
	...
	case MTKFB_GET_DISPLAY_IF_INFORMATION:
	{
		...
		if (displayid == 0) {
			...
			DISP_PR_ERR("information for displayid: %d is not available now\n",
				    displayid);
			return -EFAULT;
		}
	...
	case MTKFB_POWEROFF:
	{
		MTKFB_FUNC();
		if (primary_display_is_sleepd()) {
			DISP_PR_INFO("[FB Driver] is still in MTKFB_POWEROFF!\n");
			return ret;
		}
	...
	case MTKFB_POWERON:
	{
		...
		if (primary_display_is_alive()) {
			DISP_PR_INFO("[FB Driver] is still in MTKFB_POWERON!\n");
			return ret;
		}
	...
	case MTKFB_CAPTURE_FRAMEBUFFER:
	{
		...
		if (ret < 0)
			DISP_PR_ERR("primary display capture framebuffer failed\n");
	...
	case MTKFB_SET_VIDEO_LAYERS:
	{
		...
		for (i = 0; i < VIDEO_LAYER_COUNT; ++i) {
			if (layerInfo[i].layer_id >= TOTAL_OVL_LAYER_NUM) {
				DISP_PR_INFO("MTKFB_SET_VIDEO_LAYERS, layer_id invalid=%d\n",
	...
	default:
		DISP_PR_INFO("%s: Not support:info=0x%p, cmd=0x%08x, arg=0x%08lx\n",
			      ...
			      unsigned long arg)
{
	...
	case COMPAT_MTKFB_GET_DISPLAY_IF_INFORMATION:
	{
		...
		if (displayid == 0) {
			...
			DISP_PR_INFO("information for displayid: %d is not available now\n",
				     displayid);
		}
	...
	case COMPAT_MTKFB_SET_OVERLAY_LAYER:
	{
		...
		if (copy_from_user(compat_layerInfo, (void __user *)arg,
			 ...
			 */
			if (primary_display_is_sleepd()) {
				DISP_PR_INFO("[FB Driver] set overlay in early suspend ,skip!\n");
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	oppo_display_aodlight_support = of_property_read_bool(pdev->dev.of_node, "oppo_display_aodlight_support");
	oppo_display_aod_ramless_support = of_property_read_bool(pdev->dev.of_node, "oppo_display_aod_ramless_support");
	oppo_display_tenbits_support = of_property_read_bool(pdev->dev.of_node, "oppo_display_tenbits_support");
	oppo_display_elevenbits_support = of_property_read_bool(pdev->dev.of_node, "oppo_display_elevenbits_support");

	if (oppo_display_aod_ramless_support) {
		oppo_display_aodlight_support = 1;
	}
#endif /*VENDOR_EDIT*/
