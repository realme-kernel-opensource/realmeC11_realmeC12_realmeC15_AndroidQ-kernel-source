			   ...
			   unsigned char *dts)
{
	...
	if (len <= 0) {
		...
		if (lcm_dts->backlight_cmdq_size > BACKLIGHT_CMDQ_SIZE) {
			DISP_PR_INFO("%s:%d: LCM backlight cmdq table overflow: %d\n",
				     __FILE__, __LINE__, len);
			return;
		}
	}
	...
	enum LCM_INTERFACE_ID lcm_id, int is_lcm_inited)
{
	...
	if (check_lcm_node_from_DT() == 0) {
		...
		if (strcmp(lcm_drv->name, plcm_name)) {
			DISP_PR_ERR("FATAL ERROR! LCM Driver defined in kernel(%s) is different with LK(%s)\n",
	...
	if (_lcm_count() == 0) {
		...
		if (plcm_name == NULL) {
			...
			if (!isLCMFound) {
				DISP_PR_ERR("FATAL ERROR: cannot find lcm driver:%s in linux kernel driver\n",
					    plcm_name);
	...
	if (plcm && lcm_param) {
		...
		DISP_PR_ERR("FATAL ERROR! kzalloc plcm and plcm->params failed\n");
		goto fail;
	}
					   ...
					   int is_lcm_inited)
{
	...
	if (_lcm_count() < 2) {
		DISP_PR_ERR("no ext lcm driver defined in linux kernel driver\n");
		return NULL;
	...
	(CONFIG_MTK_DUAL_DISPLAY_SUPPORT == 2)
			...
			if (lcm_drv == NULL) {
				DISP_PR_ERR("FATAL ERROR! Cannot find EXT_LCM Driver. The LK ext lcm name is(%s)\n",
					    plcm_name);
				return NULL;
			}
		    ...
		    int force)
{
	...
	if (lcm_drv->update) {
		...
		if (!disp_lcm_is_video_mode(plcm))
			DISP_PR_ERR("lcm is cmd mode lcm_drv->update is null\n");
		return -1;
	}
...
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/12/06, add for ramless oled mipi change */
...
unsigned int disp_lcm_ATA(struct disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->ata_check) {
			...
			DISP_PR_ERR("FATAL ERROR, lcm_drv->ata_check is null\n");
			return 0;
		}
			 ...
			 unsigned int *lcm_value)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->set_lcm_cmd) {
			...
			DISP_PR_ERR("FATAL ERROR, lcm_drv->set_lcm_cmd is null\n");
			return -1;
		}
