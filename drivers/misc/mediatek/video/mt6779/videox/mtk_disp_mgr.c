...
int _ioctl_prepare_present_fence(unsigned long arg)
{
	...
	if (DISP_SESSION_TYPE(pf.session_id) != DISP_SESSION_PRIMARY) {
		DISP_PR_INFO("non-primary ask for present fence! session=0x%08x\n",
			     pf.session_id);
...
static int input_config_preprocess(struct disp_frame_cfg_t *cfg)
{
	...
	if (cfg->input_layer_num == 0 || cfg->input_layer_num >
		...
		DISP_PR_ERR("set_%s_buffer, config_layer_num invalid = %d, max_layer_num = %d!\n",
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (c->layer_enable) {
			...
			if (!dst_mva) {
				DISP_PR_ERR("disable L%d because of no valid mva\n",
					    c->layer_id);
				c->layer_enable = 0;
				is_err = 1;
			}
			...
			if (!src_fence && c->src_fence_fd != -1) {
				DISP_PR_ERR("error to get src_fence from fd%d\n",
					    c->src_fence_fd);
				is_err = 1;
			}
...
static int output_config_preprocess(struct disp_frame_cfg_t *cfg)
{
	...
	DISPFENCE("S+O/%s%d/L%d/idx%u/L%d/idx%u/(%u,%u,%ux%u)/%s/%u/%u/pa0x%08lx/mva0x%08lx/t%d/sec%d\n",
		  disp_session_type_str(session), DISP_SESSION_DEV(session),
...
static long __frame_queue_config(unsigned long arg)
{
	...
	if (cfg->prev_present_fence_fd != -1) {
		...
		if (!present_fence) {
			DISP_PR_ERR("error to get prev_present_fence from fd %d\n",
