...
int primary_display_switch_esd_mode(int mode)
{
	...
	if (!node) {
		DISP_PR_ERR("[ESD][%s] can't find DSI_TE eint compatible node\n",
			    __func__);
		return ret;
	}
	/* switch to eint mode */
	...
	while (1) {
		...
		*/
		...
		do {
			...
			DISP_PR_ERR("[ESD]esd check fail, will do esd recovery. try=%d\n",
				    i);
		...
		if (ret == 1) {
			DISP_PR_ERR("[ESD]LCM recover fail. Try time:%d. Disable esd check\n",
				    esd_try_cnt);
...
int primary_display_esd_recovery(void)
{
	...
	* add for esd recovery ramless oled
	*/
	...
	while (1) {
		...
		*/
		...
		do {
			...
			if (ret != 1)
				break;
			DISP_PR_ERR("[EXTD-ESD]esd check fail, will do esd recovery. try=%d\n",
				    i);
		...
		if (ret == 1) {
			DISP_PR_ERR("[EXTD-ESD]after esd recovery %d times, still fail, disable esd check\n",
				    esd_try_cnt);
