...
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/11/16, add for 19357 ramless oled uiready */
...
extern int display_set_wait_idle_time(unsigned int idleMs);

#define RAMLESS_AOD_PAYLOAD_SIZE	100
extern unsigned char aod_area_cmd[RAMLESS_AOD_PAYLOAD_SIZE];
#endif /* VENDOR_EDIT */
...
{
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/11/25, add for ramless aod */
	if ((disp_helper_get_option(DISP_OPT_AOD) && !disp_lcm_is_video_mode(pgc->plcm))
		|| oppo_display_aod_ramless_support)
		return 1;
#endif /*VENDOR_EDIT*/
				  ...
				  enum mtkfb_power_mode cur)
{
	if (primary_display_check_power_mode() != cur)
		DISP_PR_ERR("AOD check error: fail to set FB power mode %s to %s(but now %s)\n",
	...
	if (primary_display_is_video_mode()) {
		...
		if (islcmconnected)
			dpmgr_path_build_cmdq(pgc->dpmgr_handle,
					      ...
					      CMDQ_WAIT_LCM_TE, 0);
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/02/11, add for ramless aod */
		if (oppo_display_aod_ramless_support && rlta_enable) {
				dpmgr_path_build_cmdq(pgc->dpmgr_handle,
						  pgc->cmdq_handle_trigger,
						  CMDQ_WAIT_LCM_TE, 0);
				dpmgr_path_build_cmdq(pgc->dpmgr_handle,
				       ...
				       enum DISP_MODULE_ENUM after_engine)
{
	...
	ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_SCREEN_CAPTURE,
			    &cmdq_wait_handle);
	if (ret) {
		DISP_PR_ERR("dl_to_dc capture:Fail to create cmdq wait handle\n");
		ret = -1;
		goto out;
	}
...
static struct disp_internal_buffer_info *allocat_decouple_buffer(int size)
{
	...
	if (!buf_info) {
		DISP_PR_ERR("Fatal error, kzalloc internal buffer info failed!\n");
		kfree(buf_info);
		return NULL;
	}
...
primary_display_change_lcm_resolution(unsigned int width, unsigned int height)
{
	...
	if (primary_display_is_video_mode()) {
		DISP_PR_INFO("Warning!!!Video Mode can't support multiple resolution!\n");
		return -1;
	}
...
static int decouple_mirror_update_rdma_config_thread(void *data)
{
	...
	while (1) {
		...
		if (ret == 0) {
			...
			DISP_PR_INFO("wait decouple_update_rdma_event interrupted , ret = %d\n",
				     ret);
		}
		if (kthread_should_stop())
			break;
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, add for 19357 ramless aod */
	if (oppo_display_aod_ramless_support && rlta_enable) {
		primary_display_def_dst_mode = primary_display_is_video_mode();
	}
#endif /*VENDOR_EDIT*/
...
#ifndef VENDOR_EDIT
/*
 * YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, remove for ramless aod
 */
		if (disp_helper_get_option(DISP_OPT_SWITCH_DST_MODE))
...
int primary_display_release_fence_fake(void)
{
	...
	for (i = 0; i < PRIMARY_SESSION_INPUT_LAYER_COUNT; i++) {
		...
		if (fence_idx == -1) {
			DISP_PR_INFO("find fence for layer %d,addr 0x%08x fail, unregistered\n",
				     i, addr);
		} else if (fence_idx < 0) {
			DISP_PR_INFO("find fence idx for layer %d,addr 0x%08x fail,unknown\n",
				     i, addr);
		} else {
			if (layer_en)
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/12/09, add for ramless oled suspend */
	...
	if (primary_display_get_power_mode_nolock() == DOZE_SUSPEND) {
		...
		*/
		if (primary_display_get_lcm_power_state_nolock() == LCM_ON_LOW_POWER) {
			if (oppo_display_fppress_support && !doze_rec_fpd && !ds_rec_fpd) {
				if ((oppo_display_aod_ramless_support && rlta_enable)) {
					if (primary_display_is_video_mode()) {
						mtk_disp_lcm_set_hbm(0, pgc->plcm, NULL);
					} else {
						pr_info("%s Ramless is cmd mode set hbm 0\n", __func__);
					}
				} else {
					mtk_disp_lcm_set_hbm(0, pgc->plcm, NULL);
				}
...
#ifndef VENDOR_EDIT
/*
 * YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/12/09,
 * modify for ramless oled
 */
		disp_lcm_suspend(pgc->plcm);
#else /*VENDOR_EDIT*/
		if (!oppo_display_aod_ramless_support) {
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, add for ramless aod*/
		/*1.suspend->doze 2.resume->doze 3.doze_suspend->doze*/
#endif /*VENDOR_EDIT*/
		if (primary_display_get_lcm_power_state_nolock() !=
			LCM_ON_LOW_POWER) {
			#ifndef VENDOR_EDIT
			...
			*/
			...
			#else
			/*DISPCHECK("%s 1.suspend->doze 2.resume->doze\n", __func__);*/
			/*1.suspend->doze 2.resume->doze*/
			if (pgc->plcm->drv->aod) {
				if (primary_display_get_lcm_power_state_nolock() == LCM_ON) {
					/*resume->doze*/
					DISPCHECK("%s resume->doze\n", __func__);
						...
						if (oppo_display_fppress_support && !doze_rec_fpd && !ds_rec_fpd) {
							if (oppo_display_aod_ramless_support) {
								if (primary_display_is_video_mode()) {
									...
									DISPCHECK("%s disp is cmd Ramless set hbm [0]\n",__func__);
								}
							}
						}
			...
			* add for fingerprint notify frigger
			*/
			if (oppo_display_fppress_support && ds_rec_fpd) {
				if ((oppo_display_aod_ramless_support && rlta_enable)) {
					if (primary_display_is_video_mode()) {
						mtk_disp_lcm_set_hbm(1, pgc->plcm, NULL);
					} else {
						DISPCHECK("%s disp is cmd Ramless set hbm [1]\n",__func__);
					}
				} else {
					mtk_disp_lcm_set_hbm(1, pgc->plcm, NULL);
				}
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, add for ramless aod */
			/*1.doze->resume 2.suspend->resume 3.doze_suspend->resume*/
			DISPINFO("%s 1.doze->resume 2.suspend->resume 3.doze_suspend->resume\n", __func__);
#endif /*VENDOR_EDIT*/
			if (primary_display_get_lcm_power_state_nolock() !=
				LCM_ON_LOW_POWER) {
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, add for ramless aod */
				/*2.suspend->resume*/
				DISPINFO("%s suspend->resume\n", __func__);
#endif /*VENDOR_EDIT*/
				disp_lcm_resume(pgc->plcm);
			} else {
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15, add for ramless aod */
				/*1.doze->resume 3.doze_suspend->resume*/
				DISPINFO("%s 1.doze->resume 3.doze_suspend->resume\n", __func__);
				if ((oppo_display_aod_ramless_support && rlta_enable)) {
					if(primary_display_get_prev_power_mode_nolock() == DOZE) {
						DISPCHECK("%s 1.doze->resume C2V\n", __func__);
						primary_display_switch_aod_mode(1);
					}else {
						DISPCHECK("%s 3.doze_suspend->resume\n", __func__);
...
#ifndef VENDOR_EDIT
/*
 * YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/15,
 * modify for ramless aod
 */
		...
		if((oppo_display_aod_ramless_support && rlta_enable))
		{
			if (primary_display_get_power_mode_nolock() == DOZE)
			{
				...
				DISPCHECK("[DOZE]AOD_RAMELESS cmd/video mode=%d\n", primary_display_is_video_mode());
			...
			else if(primary_display_get_power_mode_nolock() == FB_RESUME)
			{
				...
				DISPCHECK("[FB_RESUME]AOD_RAMELESS cmd/video mode=%d\n", primary_display_is_video_mode());
				dpmgr_path_set_video_mode(pgc->dpmgr_handle, primary_display_is_video_mode());
			}
		}
		else
		...
		if (primary_display_is_decouple_mode()) {
			if (pgc->ovl2mem_path_handle)
				...
				DISP_PR_ERR("in DC mode but no ovl2mem_path_handle\n");
		}
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/02/07, add for 19357 ramless aod */
		if ((oppo_display_aod_ramless_support && rlta_enable))
		{
			if(primary_display_get_power_mode_nolock() == DOZE)
			{
				data_config->dispif_config.dsi.mode = pgc->plcm->params->dsi.mode;
			}
			else if(primary_display_get_power_mode_nolock() == FB_RESUME)
			{
				data_config->dispif_config.dsi.mode = pgc->plcm->params->dsi.mode;
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		mmprofile_log_ex(ddp_mmp_get_events()->primary_resume,
				 MMPROFILE_FLAG_PULSE, 1, 4);
		DISP_PR_ERR("[POWER]Fatal error, we didn't start display path but it's already busy\n");
		ret = -1;
		/* goto done; */
	}
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		...
		DISP_PR_ERR("[POWER]Fatal error, we didn't trigger display path but it's already busy\n");
		ret = -1;
		/* goto done; */
	}
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume,
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/02/20, add for ramless aod */
	if ((oppo_display_aod_ramless_support && rlta_enable)) {
		if(primary_display_get_power_mode_nolock() == FB_RESUME) {
			display_set_wait_idle_time(50);
		}
		if(primary_display_get_power_mode_nolock() == DOZE) {
			display_set_wait_idle_time(300);
		}
		primary_display_idlemgr_kick(__func__, 0);
	}
#endif /*VENDOR_EDIT*/
...
int primary_display_aod_backlight(int level)
{
	...
	#ifndef VENDOR_EDIT
	/*
	 * Jian.Zhou@PSW.MM.Display.LCD.Stability, 2019/10/23,
...
int primary_display_start(void)
{
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		DISP_PR_ERR("Fatal error, we didn't trigger display path but it's already busy\n");
		ret = -1;
		goto done;
	}
					  ...
					  int need_merge)
{
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE ||
		...
		if (pgc->need_trigger_ovl1to2 == 0) {
			DISP_PR_INFO("There is no output config when directlink mirror!!\n");
		} else {
	...
	if (cfg->setter != SESSION_USER_INVALID) {
		...
		if (o_mode == DISP_SESSION_DIRECT_LINK_MODE) {
			if (bypass) {
				...
				DISP_PR_INFO("request DC without a rdma frame\n");
					  ...
					  *session_input)
{
	...
	if (sizeof(session_input->config) != sizeof(frame_cfg->input_cfg)) {
		DISP_PR_INFO("session input config size not equal frame config size\n");
		return -1;
	}
...
/*Mark.Yao@PSW.MM.Display.LCD.Stable,2019-05-10 add for dc backlight feature */
	...
	if (oppo_display_fppress_support && !ds_rec_fpd && !doze_rec_fpd) {
		if (cfg->hbm_en > 0) {
				   ...
				   int block)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISP_PR_INFO("primary display switch from %s to %s in suspend state!\n",
			     session_mode_str(pgc->session_mode),
...
int primary_display_force_set_vsync_fps(unsigned int fps, unsigned int scenario)
{
	...
	if (!primary_display_is_video_mode()) {
		DISP_PR_INFO("currently display is not video mode. fps changing fail\n");
		return -1;
	}
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	if (ret) {
		DISP_PR_INFO("fail to create primary cmdq handle for backlight\n");
		return -1;
	}
...
int primary_display_set_lcm_hbm(bool en)
{
	...
	if (state == -1)
		return -EINVAL;
	else if (state == en || fp_silence_mode)
		return 0;

#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2020/01/29, add for ramless aod hbm */
if ((oppo_display_aod_ramless_support && rlta_enable)) {
		if (!primary_display_is_video_mode()) {
...
#ifdef VENDOR_EDIT
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/11/07, add for 19357 hbm wait */
	...
	if (pgc->state == DISP_SLEPT) {
		...
		if (primary_display_cmdq_enabled()) {
			if (primary_display_is_video_mode()) {
				...
				#ifndef VENDOR_EDIT
				/*
				 * YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/12/23,
				 * modify for ramless bl set
				 */
				disp_lcm_set_backlight(pgc->plcm, NULL, level);
				#else /*VENDOR_EDIT*/
				if(oppo_display_aod_ramless_support){
			 ...
			 unsigned int *lcm_value)
{
	...
	if (ret) {
		DISP_PR_INFO("fail to create primary cmdq handle for setlcmcmd\n");
		return -1;
	}
...
#ifdef VENDOR_EDIT
/*
 * Zhijun.Ye@PSW.MM.Display.LCD.Stability, 2020/02/10,
 * add for skipping mipi clk change while display is suspended
 */
	if (pgc->state == DISP_SLEPT) {
...
#ifdef VENDOR_EDIT
	/*
	 * Hao.Lin@PSW.MM.Display.LCD.Stability, 2019/12/19,
	 * modify for close dynamic osc switch
	 */
	//dsi_osc_change(0, en);
#endif /*VENDOR_EDIT*/
...
/*Jian.Zhou@PSW.MM.Display.LCD.Stability, 2019/10/23, add aod related functions*/
...
/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/12/06, add for ramless oled mipi dynamic change */
...
int _set_aod_area_by_cmdq(unsigned char *area)
{
	...
	if (!primary_display_is_video_mode()) {
		...
		_cmdq_handle_clear_dirty(cmdq_handle_area);
		...
		_cmdq_flush_config_handle_mira(cmdq_handle_area, 1);
...
int primary_display_resolution_test(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(width_array); i++) {
		...
		if (dpmgr_path_is_busy(pgc->dpmgr_handle))
			DISP_PR_ERR("[display_test]==>Fatal error, we didn't trigger display path but it's already busy\n");
