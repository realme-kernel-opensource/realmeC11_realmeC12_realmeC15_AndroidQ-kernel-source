...
bool oppo_display_aodlight_support;
bool oppo_display_aod_ramless_support;
bool oppo_display_tenbits_support;
bool oppo_display_elevenbits_support;
...
int primary_display_read_serial(char addr, uint64_t *buf, int lenth)
{
  return 0;
}
...
static ssize_t oppo_set_aod_light_mode(struct device *dev,
	...
	if (oppo_display_aodlight_support) {
		...
		/* YongPeng.Yi@PSW.MM.Display.LCD.Stability, 2019/11/25, add for 19357 fake aod */
