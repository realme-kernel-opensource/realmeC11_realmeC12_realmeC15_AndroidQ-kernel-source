...
#endif
#if defined(CONFIG_MTK_CCCI_DEVICES)
...
unsigned int g_eint_pmic_num = 193;
...
const char *pmic_interrupt_status_name[PMIC_INT_REG_NUMBER][PMIC_INT_REG_WIDTH] = {
	...
	[13] = " INT_VAUD18_OC",
	[14] = " INT_VIO18_OC",
	[15] = " INT_VSRAM_PROC1_OC",
	},
	{
	[0] = " INT_VSRAM_PROC2_OC",
	[1] = " INT_VSRAM_OTHERS_OC",
	[2] = " INT_VSRAM_MD_OC",
	[3] = " INT_VEMC_OC",
	[4] = " INT_VSIM1_OC",
	[5] = " INT_VSIM2_OC",
...
static struct pmic_sp_irq buck_irqs[][PMIC_INT_WIDTH] = {
	{
		PMIC_SP_IRQ_GEN(1, 1, INT_VPU_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VCORE_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VGPU11_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VGPU12_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VMODEM_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VPROC1_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VPROC2_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VS1_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VS2_OC),
...
static struct pmic_sp_irq ldo_irqs[][PMIC_INT_WIDTH] = {
	{
		...
		PMIC_SP_IRQ_GEN(1, 1, INT_VCN33_1_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VCN33_2_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VCN13_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VCN18_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VA09_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VCAMIO_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VA12_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VAUX18_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VAUD18_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VIO18_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VSRAM_PROC1_OC),
	},
	{
		PMIC_SP_IRQ_GEN(1, 1, INT_VSRAM_PROC2_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VSRAM_OTHERS_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VSRAM_MD_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VEMC_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VSIM1_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VSIM2_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VUSB_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VRFCK_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VBBCK_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VBIF28_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VIBR_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VIO28_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VM18_OC),
		PMIC_SP_IRQ_GEN(1, 1, INT_VUFS_OC),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
	},
};
...
static struct pmic_sp_irq bm_irqs[][PMIC_INT_WIDTH] = {
	{
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_BAT_H),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_BAT_L),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_CUR_H),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_CUR_L),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_ZCV),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_N_CHARGE_L),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_IAVG_H),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_IAVG_L),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_DISCHARGE),
		PMIC_SP_IRQ_GEN(1, 0, INT_FG_CHARGE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
	},
	{
		...
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(1, 0, INT_BATON_BAT_IN),
		PMIC_SP_IRQ_GEN(1, 0, INT_BATON_BAT_OUT),
		PMIC_SP_IRQ_GEN(1, 1, INT_BIF),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
		PMIC_SP_IRQ_GEN(0, 0, NO_USE),
...
static struct pmic_sp_irq hk_irqs[][PMIC_INT_WIDTH] = {
	{
		...
		PMIC_SP_IRQ_GEN(1, 1, INT_BAT2_H),
		PMIC_SP_IRQ_GEN(1, 1, INT_BAT2_L),
		PMIC_SP_IRQ_GEN(1, 1, INT_BAT_TEMP_H),
		PMIC_SP_IRQ_GEN(1, 1, INT_BAT_TEMP_L),
		PMIC_SP_IRQ_GEN(1, 1, INT_THR_H),
		PMIC_SP_IRQ_GEN(1, 1, INT_THR_L),
...
struct oc_int_work_t {
	int oc_intNo;
	const char *name;
	struct work_struct work;
};

static DEFINE_MUTEX(oc_int_mutex);
static struct oc_int_work_t g_oc_work;

static void oc_int_work(struct work_struct *work)
{
	const char *name;
	int intNo;
	int enable_reg, oc_reg;
	unsigned int spNo, sp_conNo, sp_irqNo;
	struct oc_int_work_t *oc_work =
		container_of(work, struct oc_int_work_t, work);

	mutex_lock(&oc_int_mutex);
	intNo = oc_work->oc_intNo;
	name = oc_work->name;
	...
	switch (intNo) {
	case INT_VBIF28_OC:
		enable_reg = PMIC_RG_LDO_VBIF28_EN;
		oc_reg = PMIC_RGS_VBIF28_OC_STATUS;
		break;
	case INT_VIO28_OC:
		enable_reg = PMIC_RG_LDO_VIO28_EN;
	...
	while (pmic_get_register_value(oc_reg)) {
		pmic_set_register_value(enable_reg, 0);
		msleep(1000);
		pmic_set_register_value(enable_reg, 1);
		mdelay(1);
	}
	mdelay(5);
...
void pmic_enable_interrupt(enum PMIC_IRQ_ENUM intNo, unsigned int en, char *str)
{
	...
	if (en == 1)
		pmic_config_interface(enable_reg, 1, 0x1, sp_irqNo);
	else if (en == 0)
		pmic_config_interface(enable_reg, 0, 0x1, sp_irqNo);
#endif
...
static void pmic_int_handler(void)
{
	...
	top_int_status = upmu_get_reg_value(MT6359_TOP_INT_STATUS0);
