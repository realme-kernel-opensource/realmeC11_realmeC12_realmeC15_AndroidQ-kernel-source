...
static void lcm_get_params(struct LCM_PARAMS *params)
{
    ...
    //Hongwu.Wei@ODM_WT.MM.LCD,2020/04/04, add /proc/devinfo/lcd node
    register_device_proc("lcd", "nt36525b", "boe_novtek");
}
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_err("[ HW check backlight nt36525b+boe]level=%d para_list[0]=%x,para_list[1]=%x\n",level,bl_level[0].para_list[0],bl_level[0].para_list[1]);
	//MDELAY(5);
...
struct LCM_DRIVER nt36525b_hlt_psc_boe_hdp_dsi_vdo_lcm_drv = {
	.name = "nt36525b_hlt_psc_boe_hdp_dsi_vdo_lcm",
