...
unsigned char himax_bc_reg;
early_param("lcdgateic", parse_lcdBias);

//Hao.Liang@ODM_WT.MM.Display.Lcd, 2020/6/23, identify himax_bc
static int __init parse_himax_reg(char *arg)
{

	if (!arg)
		return -EINVAL;
	//himax reg bc is 07 or 04;
	pr_info("himax_bc=%s\n",arg);
	if (strcmp(arg, "0x07") == 0) {
		pr_info("parse_himax_reg_bc is 0x07 \n");
		himax_bc_reg = 0x07;
	}
	else if (strcmp(arg, "0x04") == 0) {
		pr_info("parse_himax_reg_bc is 0x04 \n");
		himax_bc_reg = 0x04;
	}
	return 0;
}

early_param("himax_bc", parse_himax_reg);
