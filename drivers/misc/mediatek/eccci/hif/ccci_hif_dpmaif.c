...
static int dpmaif_queue_broadcast_state(struct hif_dpmaif_ctrl *hif_ctrl,
	enum HIF_STATE state, DIRECTION dir, unsigned char index)
{
#ifdef DPMAIF_DEBUG_LOG
		...
		int blocking, unsigned long time_limit)
{
	...
	for (rx_cnt = 0; rx_cnt < pit_cnt; rx_cnt++) {
		if (!blocking && time_after_eq(jiffies, time_limit)) {
			...
			"dpmaif_rx_start: timeout, cnt = %d/%d\n",
			rx_cnt, pit_cnt);
			break;
		}
...
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(-1, TAG, "dpmaif_rx_start: pit:0x%x, 0x%x, 0x%x\n",
		rxq->pit_wr_idx, rxq->pit_rd_idx,
			rxq->pit_rel_rd_idx);
	CCCI_HISTORY_LOG(-1, TAG, "dpmaif_rx_start:bat: 0x%x, 0x%x, 0x%x\n",
		rxq->bat_req.bat_wr_idx, rxq->bat_req.bat_rd_idx,
		rxq->bat_req.bat_rel_rd_idx);
#ifdef HW_FRG_FEATURE_ENABLE
...
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(dpmaif_ctrl->md_id, TAG,
		"dpmaif_tx_release hw cnt = 0x%x, 0x%x, 0x%x\n",
	...
	struct sk_buff *skb, int skb_from_pool, int blocking)
{
	...
	iph = ip_hdr(skb);
	if (skb->mark & UIDMASK && iph->protocol == IPPROTO_UDP)
		prio_count = 0x1000;
...
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_TAG_LOG(-1, TAG, "dpmaif:dpmaif_late_init\n");
#else
	CCCI_DEBUG_LOG(-1, TAG, "dpmaif:dpmaif_late_init\n");
#endif
...
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_TAG_LOG(-1, TAG, "dpmaif:dpmaif_late_init end\n");
#else
	CCCI_DEBUG_LOG(-1, TAG, "dpmaif:dpmaif_late_init end\n");
#endif
	return 0;
}
...
void dpmaif_hw_reset(unsigned char md_id)
{
	...
	/* DPMAIF HW reset */
	CCCI_DEBUG_LOG(md_id, TAG, "dpmaif_hw_reset:rst dpmaif\n");
	...
	CCCI_BOOTUP_LOG(md_id, TAG, "dpmaifa_hw_reset:clear reset\n");
	/* reset dpmaif clr */
	...
	CCCI_BOOTUP_LOG(md_id, TAG, "dpmaif_hw_reset:done\n");
...
int ccci_dpmaif_hif_init(unsigned char hif_id, unsigned char md_id)
{
	...
	if (hif_ctrl == NULL) {
		CCCI_ERROR_LOG(-1, TAG,
			"ccci_dpmaif_hif_init:alloc hif_ctrl fail\n");
		ret = -1;
