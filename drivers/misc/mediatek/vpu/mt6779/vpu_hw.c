...
#if 0
	PREPARE_VPU_CLK(clk_apu_mdla_apb_cg);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b0);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b1);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b2);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b3);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b4);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b5);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b6);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b7);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b8);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b9);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b10);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b11);
	PREPARE_VPU_CLK(clk_apu_mdla_cg_b12);
#endif
...
#if 0
	UNPREPARE_VPU_CLK(clk_apu_mdla_apb_cg);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b0);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b1);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b2);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b3);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b4);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b5);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b6);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b7);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b8);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b9);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b10);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b11);
	UNPREPARE_VPU_CLK(clk_apu_mdla_cg_b12);
#endif
...
info18_out:
	...
	if (normal_check_done == 1) {
		vpu_trace_dump("VPU%d VPU_REQ_DO_CHECK_STATE OK", core);
		#ifndef VENDOR_EDIT
		/*jiangtao.ren@Camera add for vpu timeout of pdaf 20200111*/
...
int vpu_hw_processing_request(int core, struct vpu_request *request)
{
	...
	*/
	...
	ktime_get_ts(&start);
	if (g_vpu_log_level > Log_ALGO_OPP_INFO)
	LOG_INF("[0x%lx]:d2d start ", (unsigned long)request->request_id);
	/* 3. wait until done */
	ret = wait_command(core);
	if (ret == -ERESTARTSYS)
		is_hw_fail = false;
...
int vpu_set_power_parameter(uint8_t param, int argc, int *args)
{
	...
	case VPU_POWER_HAL_CTL:
	{
        ...
        #ifdef VENDOR_EDIT
        /*Qiang.Bi@Camera.App, reduece ageing log ,20200110*/
        if(g_vpu_log_level > Log_ALGO_OPP_INFO){
            LOG_INF("[vpu]POWER_HAL_LOCK+core:%d, maxb:%d, minb:%d\n",
                vpu_lock_power.core, vpu_lock_power.max_boost_value,vpu_lock_power.min_boost_value);
        }
        #else
        LOG_INF("[vpu]POWER_HAL_LOCK+core:%d, maxb:%d, minb:%d\n",
            vpu_lock_power.core, vpu_lock_power.max_boost_value,vpu_lock_power.min_boost_value);
