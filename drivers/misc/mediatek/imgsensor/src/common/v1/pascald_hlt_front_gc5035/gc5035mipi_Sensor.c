...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = PASCALD_HLT_FRONT_GC5035_SENSOR_ID,
...
static kal_uint16 read_cmos_sensor_hlt_otp(kal_uint32 addr)
{
    kal_uint16 get_byte=0;
...
static int read_gc5035_hlt_module_id(void)
{
	kal_uint16 gc5035_hlt_moduleid_id=0x0;
	gc5035_hlt_moduleid_id = read_cmos_sensor_hlt_otp(0x0000);
	printk("gc5035_hlt_moduleid_id = %d\n",gc5035_hlt_moduleid_id);
	return gc5035_hlt_moduleid_id;
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == GC5035_SENSOR_ID) {
				...
				if(read_gc5035_hlt_module_id() == 0x09)
				{
					*sensor_id = imgsensor_info.sensor_id;
					hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "Hlt");
					pr_debug("hlt gc5035 get_imgsensor_id *sensor_id = 0x%x\n",*sensor_id);
					//return ERROR_NONE;
				}
				else{
					*sensor_id = 0xFFFFFFFF;
					pr_debug("Hlt match fail,so truly");
