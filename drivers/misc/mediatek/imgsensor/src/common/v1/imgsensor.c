...
void IMGSENSOR_PROFILE(struct timeval *ptv, char *tag)
{
	...
	PR_INFO_IF(dumpEnable, "[%s]Profile = %lu us\n", tag, time_interval);
}
 ...
 ************************************************************************/
#define IMGSENSOR_FUNCTION_ENTRY()    /*PR_INFO_IF(dumpEnable, "[%s]:E\n",__FUNCTION__)*/
#define IMGSENSOR_FUNCTION_EXIT()     /*PR_INFO_IF(dumpEnable, "[%s]:X\n",__FUNCTION__)*/
...
imgsensor_sensor_open(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (psensor_func &&
		...
		if (ret != IMGSENSOR_RETURN_SUCCESS) {
			PR_ERR_IF(dumpEnable, "[%s]", __func__);
			return -EIO;
		}
		...
		if (ret != ERROR_NONE) {
			...
			PR_ERR_IF(dumpEnable, "SensorOpen fail");
		} else {
	...
	MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	if (psensor_func &&
		...
		if (ret != ERROR_NONE)
			PR_ERR_IF(dumpEnable, "[%s] SensorGetInfo failed\n", __func__);
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		PR_INFO_IF(dumpEnable, "Fail to get sensor ID %x\n", sensorID);
		err = ERROR_SENSOR_CONNECT_FAIL;
	} else {
		PR_INFO_IF(dumpEnable, " Sensor found ID = 0x%x\n", sensorID);
		err = ERROR_NONE;
	}

	if (err != ERROR_NONE)
		PR_INFO_IF(dumpEnable, "ERROR: No imgsensor alive\n");
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	#ifdef VENDOR_EDIT
	//wenui.chen@Cam.Drv, 20200416, add for pascal camera drvier porting
	if (pascal_project() == 1) {
	...
	if (get_search_list) {
		...
		if (psensor_list_config) {
			...
			*(psensor_list_config+strlen(sensor_configs)-2) = '\0';
			...
			PR_INFO_IF(dumpEnable, "sensor_list %s\n", psensor_list_config);
	...
	/*PR_DEBUG_IF(dumpEnable, "get_search_list %d,\n %d %d %d %d\n %d %d %d %d\n",
	 *   get_search_list,
	 *   orderedSearchList[0],
	 *   orderedSearchList[1],
	 *   orderedSearchList[2],
	...
	for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
		/*PR_DEBUG_IF(dumpEnable, "orderedSearchList[%d]=%d\n",
		...
		if (pSensorList[drv_idx].init) {
			...
			if (psensor->pfunc) {
				...
				PR_ERR_IF(dumpEnable,
				    "ERROR:NULL g_pInvokeSensorFunc[%d][%d]\n",
				    psensor->inst.sensor_idx,
				    drv_idx);
			}
		} else {
			PR_ERR_IF(dumpEnable, "ERROR:NULL sensor list[%d]\n", drv_idx);
		}
...
static inline int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	if (pSensorGetInfo == NULL ||
		...
		PR_DEBUG_IF(dumpEnable, "[CAMERA_HW] NULL arg.\n");
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorGetInfo->SensorId);
	if (psensor == NULL) {
		PR_DEBUG_IF(dumpEnable, "[CAMERA_HW] NULL psensor.\n");
		return -EFAULT;
	}
	...
	if (pInfo == NULL || pConfig == NULL) {
		kfree(pInfo);
		kfree(pConfig);
		PR_ERR_IF(dumpEnable, " ioctl allocate mem failed\n");
		return -ENOMEM;
	}
	...
	if (copy_to_user((void __user *)(pSensorGetInfo->pInfo),
		...
		PR_ERR_IF(dumpEnable, "[CAMERA_HW][info] ioctl copy to user failed\n");
		if (pInfo != NULL)
			kfree(pInfo);
		if (pConfig != NULL)
			kfree(pConfig);
	...
	if (copy_to_user((void __user *) (pSensorGetInfo->pConfig),
		...
		PR_DEBUG_IF(dumpEnable, "[CAMERA_HW][config] ioctl copy to user failed\n");
	...
	MUINT32 *temp)
{
	...
	*temp  = 0;
	...
	if (psensor_inst->state != IMGSENSOR_STATE_CLOSE) {
		...
		PR_DEBUG_IF(dumpEnable, "senDevId(%d), temperature(%d)\n", senDevId, *temp);
	...
	struct work_struct *data)
{
	...
	PR_DEBUG_IF(dumpEnable, "Temperature Meter Report.\n");
	...
	PR_INFO_IF(dumpEnable, "senDevId(%d), valid(%d), temperature(%d)\n",
				DUAL_CAMERA_MAIN_SENSOR, valid[0], temp[0]);

	if (ret != ERROR_NONE)
		PR_ERR_IF(dumpEnable, "Get Main cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		PR_ERR_IF(dumpEnable, "Get Sub cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		PR_ERR_IF(dumpEnable, "Get Main2 cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		PR_ERR_IF(dumpEnable, "Get Sub2 cam temperature error(%d)!\n", ret);
...
struct match_hardwareinfo hardwareinfo_camera_name[] = {
	{ SENSOR_DRVNAME_PASCALD_TRULY_MAIN_OV13B10, "pascald_truly_main_ov13b10", " 0x560D43"},
	{ SENSOR_DRVNAME_PASCALD_QTECH_MAIN_OV13B10, "pascald_qtech_main_ov13b10", " 0x560D44"},
	{ SENSOR_DRVNAME_PASCALD_HLT_FRONT_GC5035,   "pascald_hlt_front_gc5035", " 0x5036"},
	/* wenhui.Chen@Cam.Drv, 20200430, sensor porting for shengtai GC5035*/
	{ SENSOR_DRVNAME_PASCALD_SHENGTAI_FRONT_GC5035,   "pascald_shengtai_front_gc5035", " 0x5037"},
	/* Shipei.Chen@Cam.Drv, 20200414,  add for [add hardware_info for factory]*/
	{ SENSOR_DRVNAME_PASCALD_HLT_FRONT_S5K4H7,   "pascald_hlt_front_s5k4h7", " 0x487B"},
	{ SENSOR_DRVNAME_PASCALD_HLT_DEPTH_GC02K0B, "pascald_hlt_depth_gc02k0b", " 0x2385"},
	{ SENSOR_DRVNAME_PASCALD_HLT_DEPTH_GC02M1B, "pascald_hlt_depth_gc02m1b", " 0x02E1"},
	{ SENSOR_DRVNAME_PASCAL_HLT_MONO_GC02M1B, "pascald_hlt_mono_gc02m1b", " 0x02E2"},
	{ SENSOR_DRVNAME_PASCAL_CXT_MONO1_GC02M1B, "pascal_cxt_mono1_gc02m1b", " 0x02E4"},
	{ SENSOR_DRVNAME_PASCAL_CXT_MONO_OV02B1B, "pascal_cxt_mono_ov02b1b", " 0x002E"},
	/* Shipei.Chen@Cam.Drv, 20200426,  modify for hardwareinfo!*/
	{ SENSOR_DRVNAME_PASCAL_SHENGTAI_MONO1_OV02B1B, "pascal_shengtai_mono1_ov02b1b", " 0x002B"},
	{ SENSOR_DRVNAME_PASCAL_LHYX_MONO1_OV02B1B, "pascal_lhyx_mono1_ov02b1b", " 0x002C"},
	{ SENSOR_DRVNAME_PASCALD_CXT_DEPTH_GC02M1B, "pascald_cxt_depth_gc02m1b", " 0x00e3"},
	{ SENSOR_DRVNAME_PASCALD_LHYX_DEPTH_OV02B1B, "pascald_lhyx_depth_ov02b1b", " 0x002d"},
	{ SENSOR_DRVNAME_PASCAL_SHENGTAI_WIDE_OV8856, "pascal_shengtai_wide_ov8856", " 0x885A"},
	/* wang.Gao@Cam.Drv, 20200529,sensor porting for shengtai2 GC5035*/
	{ SENSOR_DRVNAME_PASCALD_SHENGTAI2_FRONT_GC5035,   "pascald_shengtai2_front_gc5035", " 0x5038"},
	{ SENSOR_DRVNAME_PASCALE_HLT_MACRO_OV02B10,   "pascale_hlt_macro_ov02b10", " 0x002F"},
	{"NULL", "UNKNOWN", "0"},
};
#endif /* ODM_WT_EDIT */
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	if (pSensorGetInfo == NULL ||
		...
		PR_INFO_IF(dumpEnable, "[adopt_CAMERA_HW_GetInfo2] NULL arg.\n");
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorGetInfo->SensorId);
	if (psensor == NULL) {
		PR_INFO_IF(dumpEnable, "[adopt_CAMERA_HW_GetInfo2] NULL psensor.\n");
		return -EFAULT;
	}

	PR_DEBUG_IF(dumpEnable, "[adopt_CAMERA_HW_GetInfo2]Entry%d\n",
	...
	if (copy_to_user(
		...
		PR_DEBUG_IF(dumpEnable, "[CAMERA_HW][info] ioctl copy to user failed\n");
		ret = -EFAULT;
		goto IMGSENSOR_GET_INFO_RETURN;
	}
	...
	PR_INFO_IF(dumpEnable, "i = %d, hardwareinfo_camera_name:%s, psensor->inst.psensor_name:%s, sensor_idx:%s",
	...
	if(pSensorGetInfo->SensorId == 2){//mono&&wide
		#ifdef VENDOR_EDIT
		/* Shipei.Chen@Cam.Drv, 20200525,  modify for hardware info!*/
		if (pascal_project() == 1) {
	...
	if(pSensorGetInfo->SensorId == 3){//mono
		if (pascal_project() == 3) {
	...
	if (copy_to_user(
		...
		PR_DEBUG_IF(dumpEnable, "[CAMERA_HW][Resolution] ioctl copy to user failed\n");
		ret = -EFAULT;
		goto IMGSENSOR_GET_INFO_RETURN;
	}
...
static inline int adopt_CAMERA_HW_Control(void *pBuf)
{
	...
	if (pSensorCtrl == NULL) {
		PR_ERR_IF(dumpEnable, "[adopt_CAMERA_HW_Control] NULL arg.\n");
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorCtrl->InvokeCamera);
	if (psensor == NULL) {
		PR_ERR_IF(dumpEnable, "[adopt_CAMERA_HW_Control] NULL psensor.\n");
		return -EFAULT;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	if (pFeatureCtrl  == NULL) {
		PR_ERR_IF(dumpEnable, " NULL arg.\n");
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pFeatureCtrl->InvokeCamera);
	if (psensor == NULL) {
		PR_ERR_IF(dumpEnable, "[adopt_CAMERA_HW_FeatureControl] NULL psensor.\n");
		return -EFAULT;
	}
	...
	if (pFeatureCtrl->FeatureId == SENSOR_FEATURE_SINGLE_FOCUS_MODE ||
		...
		if (copy_from_user((void *)&FeatureParaLen,
			...
			PR_ERR_IF(dumpEnable, " ioctl copy from user failed\n");
			return -EFAULT;
		}
	...
	case SENSOR_FEATURE_SET_SENSOR_SYNC:
	case SENSOR_FEATURE_SET_ESHUTTER_GAIN:
		PR_DEBUG_IF(dumpEnable, "[kd_sensorlist]enter kdSetExpGain\n");
		if (copy_from_user(
		    (void *)pFeaturePara,
		    (void *) pFeatureCtrl->pFeaturePara,
		    FeatureParaLen)) {
	...
	case SENSOR_FEATURE_GET_EV_AWB_REF:
	{
		...
		*(pFeaturePara_64) = (uintptr_t)pAeAwbRef;
		...
		if (copy_to_user(
		    ...
		    (void *)pAeAwbRef,
		    sizeof(struct SENSOR_AE_AWB_REF_STRUCT))) {
			PR_DEBUG_IF(dumpEnable, "[CAMERA_HW]ERROR: copy_to_user fail\n");
		}
		kfree(pAeAwbRef);
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
	{
		...
		if (sizeof(kal_uint8) * u4RegLen > PDAF_DATA_SIZE) {
			kfree(pFeaturePara);
			PR_DEBUG_IF(dumpEnable, "check: u4RegLen > PDAF_DATA_SIZE\n");
			return -EINVAL;
		}
		...
		if (copy_from_user(
			...
			PR_ERR_IF(dumpEnable, "[CAMERA_HW]ERROR: copy from user fail\n");
		}
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
	{
		...
		if (buf_size > PDAF_DATA_SIZE) {
			kfree(pFeaturePara);
			PR_DEBUG_IF(dumpEnable, "check: buf_size > PDAF_DATA_SIZE\n");
			return -EINVAL;
		}
		pPdaf_data = kmalloc(
...
imgsensor_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO:
	{
		...
		struct IMGSENSOR_GET_CONFIG_INFO_STRUCT __user *data;
		int err;
		/*PR_DEBUG_IF(dumpEnable, "CAOMPAT_KDIMGSENSORIOC_X_GETINFO E\n"); */
	...
	case COMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL:
	{
		...
		/* PR_DEBUG_IF(dumpEnable, "CAOMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL\n"); */
	...
	case COMPAT_KDIMGSENSORIOC_X_CONTROL:
	{
		...
		PR_DEBUG_IF(dumpEnable, "[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_CONTROL\n");
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO2:
	{
		...
		PR_DEBUG_IF(dumpEnable, "[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_GETINFO2\n");
	...
	unsigned long a_u4Param)
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (pBuff == NULL) {
			PR_DEBUG_IF(dumpEnable, "[CAMERA SENSOR] ioctl allocate mem failed\n");
			i4RetValue = -ENOMEM;
...
#define ISP_CLK_HIGH   490
	...
	case KDIMGSENSORIOC_X_SET_CURRENT_SENSOR:
		...
		break;
	default:
		PR_DEBUG_IF(dumpEnable, "No such command %d\n", a_u4Command);
		i4RetValue = -EPERM;
		break;
	}
	...
	if ((_IOC_READ & _IOC_DIR(a_u4Command)) &&
		    copy_to_user((void __user *) a_u4Param,
						...
						_IOC_SIZE(a_u4Command))) {
		kfree(pBuff);
		PR_DEBUG_IF(dumpEnable, "[CAMERA SENSOR] ioctl copy to user failed\n");
		i4RetValue =  -EFAULT;
		goto CAMERA_HW_Ioctl_EXIT;
	}
...
static inline int imgsensor_driver_register(void)
{
	...
	if (alloc_chrdev_region(&dev_no, 0, 1, IMGSENSOR_DEV_NAME)) {
		PR_DEBUG_IF(dumpEnable, "[CAMERA SENSOR] Allocate device no failed\n");
	...
	if (gpimgsensor_cdev ==  NULL) {
		...
		PR_DEBUG_IF(dumpEnable, "[CAMERA SENSOR] Allocate mem for kobject failed\n");
		return -ENOMEM;
	}
	...
	if (cdev_add(gpimgsensor_cdev, dev_no, 1)) {
		PR_DEBUG_IF(dumpEnable, "Attatch file operation failed\n");
		unregister_chrdev_region(dev_no, 1);
		return -EAGAIN;
	}
	...
	if (IS_ERR(gpimgsensor_class)) {
		...
		PR_DEBUG_IF(dumpEnable, "Unable to create class, err = %d\n", ret);
		return ret;
	}
...
{
	/*PR_DEBUG_IF(dumpEnable, "mmsys_clk_change_cb ori:%d, new:%d, current_mmsys_clk %d\n",
	 *		ori_clk_mode,
	 *		new_clk_mode,
	 *		current_mmsys_clk);
	 */
...
static int imgsensor_probe(struct platform_device *pdev)
{
	...
	if (imgsensor_driver_register()) {
		PR_ERR_IF(dumpEnable, "[CAMERA_HW] register char device failed!\n");
		return -1;
	}
...
{
	PR_INFO_IF(dumpEnable, "[camerahw_probe] start\n");

	if (platform_driver_register(&gimgsensor_platform_driver)) {
		PR_ERR_IF(dumpEnable, "failed to register CAMERA_HW driver\n");
		return -ENODEV;
	}
