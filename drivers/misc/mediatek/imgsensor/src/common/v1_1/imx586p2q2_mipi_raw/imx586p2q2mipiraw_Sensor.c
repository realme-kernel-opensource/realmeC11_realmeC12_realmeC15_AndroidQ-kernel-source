...
#include "imx586p2q2mipiraw_Sensor.h"
...
static void read_eeprom_CamInfo(void)
{
	...
	for (idx = 0; idx <12; idx++) {
		...
		LOG_INF("imx586_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	gImx586_CamInfo[0] = get_byte[0];
	gImx586_CamInfo[1] = get_byte[1];
	gImx586_CamInfo[2] = get_byte[6];
	gImx586_CamInfo[3] = get_byte[7];
	gImx586_CamInfo[4] = get_byte[8];
	gImx586_CamInfo[5] = get_byte[9];
	gImx586_CamInfo[6] = get_byte[10];
	gImx586_CamInfo[7] = get_byte[11];
}

static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gImx586_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		char pusendcmd[2] = {0x00 , (char)((0xB0 + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0xA0);
		LOG_INF("imx586_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gImx586_SN[idx]);
	}

	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(IMX586_STEREO_START_ADDR_WIDE+idx)>>8,
			(char)((IMX586_STEREO_START_ADDR_WIDE + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&gOtpCheckdata[0][idx],1, 0xA0);
		LOG_INF("IMX586_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(IMX586_STEREO_START_ADDR_TELE+idx)>>8,
			(char)((IMX586_STEREO_START_ADDR_TELE + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&gOtpCheckdata[6][idx],1, 0xA0);
		LOG_INF("IMX586_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}
}
...
UINT32 IMX586P2Q2_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
