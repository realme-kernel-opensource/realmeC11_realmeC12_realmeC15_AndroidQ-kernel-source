...
#include "gc8054fmipiraw_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc8054f_camera_sensor"
#define LOG_1 LOG_INF("GC8054FMIPI, 2LANE\n")
/****************************   Modify end    *******************************************/
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{ 3264, 2448, 352, 56, 2560, 1920, 2560, 1920, 0, 0, 2560, 1920, 0, 0, 2560, 1920}  /* custom1 */
};

static struct gc8054f_otp gc8054f_otp_data = { 0 };
...
static kal_uint8 gGc8054F_SN[CAMERA_MODULE_SN_LENGTH];
static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gGc8054F_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		...
		LOG_INF("gGc8054F_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gGc8054F_SN[idx]);
	}
}
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	if(pStereodata != NULL) {
		pr_debug("gc8054f SET_SENSOR_OTP: 0x%x %d 0x%x %d\n",
		...
		if ((pStereodata->uSensorId == GC8054F_SENSOR_ID)
				&& (data_base == 0x1600)
				&& (data_length == 1561)) {
			pr_debug("gc8054f Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556],
			...
			msleep(6);
			pr_debug("gc8054fwrite_Module_data Write end\n");
		}else {
			pr_err("Invalid Sensor id:0x%x write_gm1 eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		pr_err("gc8054fwrite_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
...
static kal_uint8 gc8054f_otp_read_byte(kal_uint16 addr)
{
...
#if defined(gc8054f_otp_CUSTOMER)
static void gc8054f_otp_read_group(kal_uint16 addr, kal_uint8 *data, kal_uint16 length)
{
	kal_uint16 i = 0;
...
static void gc8054f_otp_read_sensor_info(void)
{
	kal_uint8 flag_dd = 0;
	...
	/* DPC */
	flag_dd = gc8054f_otp_read_byte(0);
	LOG_INF("flag_dd = 0x%x\n", flag_dd);
	switch (flag_dd & 0x03) {
	case 0x00:
		LOG_INF("DD is empty !!\n");
		gc8054f_otp_data.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("DD is valid!!\n");
		gc8054f_otp_data.dd_num = gc8054f_otp_read_byte(0)
						+ gc8054f_otp_read_byte(0);
		gc8054f_otp_data.dd_flag = 0x01;
		LOG_INF("total_num = %d\n", gc8054f_otp_data.dd_num);
		break;
	case 0x02:
	case 0x03:
		LOG_INF("DD is invalid !!\n");
		gc8054f_otp_data.dd_flag = 0x02;
		break;
	default:
		break;
	}
	...
	/*Register Update*/
	gc8054f_otp_data.reg_flag = gc8054f_otp_read_byte(0);
	LOG_INF("Register value flag = 0x%x\n", gc8054f_otp_data.reg_flag);
	if ((gc8054f_otp_data.reg_flag & 0x03) == 0x01)
		for (i = 0; i < 0; i++) {
			temp = gc8054f_otp_read_byte(0 + 0 * 8 * i);
			for (j = 0; j < 2; j++) {
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {// check
					gc8054f_otp_data.regs[gc8054f_otp_data.reg_num][0] = (temp >> (4 * j)) & 0x07;// page
					gc8054f_otp_data.regs[gc8054f_otp_data.reg_num][1] =
						gc8054f_otp_read_byte(0 + 0 * 8 * i + 0x10 * j);
					gc8054f_otp_data.regs[gc8054f_otp_data.reg_num][2] =
						gc8054f_otp_read_byte(0 + 0 * 8 * i + 0x10 * j);
					gc8054f_otp_data.reg_num++;
				}
			}
		}
}

static void gc8054f_otp_update_dd(void)
{
	kal_uint8 state = 0;
	kal_uint8 n = 0;

	if (0x01 == gc8054f_otp_data.dd_flag) {
		...
		write_cmos_sensor(0x01, (gc8054f_otp_data.dd_num >> 8) & 0x07);
		write_cmos_sensor(0x02, gc8054f_otp_data.dd_num & 0xff);
...
static void gc8054f_otp_update_reg(void)
{
	kal_uint8 i = 0;
	...
	if ((gc8054f_otp_data.reg_flag & 0x03) == 0x01) {
		for (i = 0; i < gc8054f_otp_data.reg_num; i++) {
			write_cmos_sensor(0xfe, gc8054f_otp_data.regs[i][0]);
			write_cmos_sensor(gc8054f_otp_data.regs[i][1], gc8054f_otp_data.regs[i][2]);
		}
	}
...
#if defined(gc8054f_otp_CUSTOMER)
static void gc8054f_otp_read_module_info(void)
{
	...
	flag_module = gc8054f_otp_read_byte(MODULE_INFO_FLAG_OFFSET);
	...
	for (index = 0; index < 2; index++)
		...
		case 0x04:
			...
			gc8054f_otp_read_group(MODULE_INFO_OFFSET + MODULE_INFO_SIZE * 8 * index, &info[0], MODULE_INFO_SIZE);
			for (i = 0; i < 6; i++)
...
static void gc8054f_otp_read_wb_info(void)
{
	kal_uint8  flag_wb = 0;
	...
	flag_wb = gc8054f_otp_read_byte(WB_INFO_FLAG_OFFSET);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x00:
			...
			gc8054f_otp_data.wb_flag = gc8054f_otp_data.wb_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("wb unit group %d is valid!!\n", index + 1);
			gc8054f_otp_read_group(WB_INFO_OFFSET + WB_INFO_SIZE * 8 * index, &wb[0], WB_INFO_SIZE);
			...
			if ((checkwb % 255 + 1) == wb[WB_INFO_SIZE - 1]) {
				gc8054f_otp_data.rg_gain = (wb[0] | ((wb[1] & 0xf0) << 4)) > 0 ?
					(wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
				gc8054f_otp_data.bg_gain = (((wb[1] & 0x0f) << 8) | wb[2]) > 0 ?
					(((wb[1] & 0x0f) << 8) | wb[2]) : 0x400;
				gc8054f_otp_data.wb_flag = gc8054f_otp_data.wb_flag | 0x01;
			} else
		...
		case 0x0c:
			...
			gc8054f_otp_data.wb_flag = gc8054f_otp_data.wb_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			...
			gc8054f_otp_data.golden_flag = gc8054f_otp_data.golden_flag | 0x00;
			break;
		case 0x40:
			LOG_INF("wb golden group %d is valid!!\n", index + 1);
			gc8054f_otp_read_group(WB_GOLDEN_INFO_OFFSET + WB_GOLDEN_INFO_SIZE * 8 * index, &golden[0], WB_GOLDEN_INFO_SIZE);
			for (i = 0; i < 4; i++) {
			...
			if ((checkgolden % 255 + 1) == golden[WB_GOLDEN_INFO_SIZE - 1]) {
				gc8054f_otp_data.golden_rg = (golden[0] | ((golden[1] & 0xf0) << 4)) > 0 ?
					(golden[0] | ((golden[1] & 0xf0) << 4)) : RG_TYPICAL;
				gc8054f_otp_data.golden_bg = (((golden[1] & 0x0f) << 8) | golden[2]) > 0 ?
					(((golden[1] & 0x0f) << 8) | golden[2]) : BG_TYPICAL;
				gc8054f_otp_data.golden_flag = gc8054f_otp_data.golden_flag | 0x01;
			} else
		...
		case 0xc0:
			...
			gc8054f_otp_data.golden_flag = gc8054f_otp_data.golden_flag | 0x02;
			break;
		default:
			break;
		}
...
static void gc8054f_otp_update_wb(void)
{
	...
	if (0x02 == gc8054f_otp_data.golden_flag) {
		return;
	}
	if (0x01 == (gc8054f_otp_data.golden_flag & 0x01)) {
		rg_typical = gc8054f_otp_data.golden_rg;
		bg_typical = gc8054f_otp_data.golden_bg;
		LOG_INF("golden_flag = %d, rg_typical = 0x%x, bg_typical = 0x%x\n",
			gc8054f_otp_data.golden_flag, rg_typical, bg_typical);
	} else {
	...
	if (0x01 == (gc8054f_otp_data.wb_flag & 0x01)) {
		r_gain_current = 2048 * rg_typical / gc8054f_otp_data.rg_gain;
		b_gain_current = 2048 * bg_typical / gc8054f_otp_data.bg_gain;
		g_gain_current = 2048;
...
static void gc8054f_otp_update(void)
{
	gc8054f_otp_update_dd();
#if defined(gc8054f_otp_CUSTOMER)
	gc8054f_otp_update_wb();
#endif
	gc8054f_otp_update_reg();
}

static void gc8054f_otp_function(void)
{
	memset(&gc8054f_otp_data, 0, sizeof(gc8054f_otp_data));
...
{
	unsigned long flags;
	kal_uint16 realtime_fps = 0, cal_shutter = 2;
	...
	imgsensor.Dgain_ratio = (shutter << 10) / cal_shutter;
	write_cmos_sensor(0x0202, cal_shutter & 0xfffe);

	LOG_INF("shutter = %d, framelength = %d, Dgain_ratio = %d\n",
		shutter, imgsensor.frame_length, imgsensor.Dgain_ratio);
}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	reg_gain = (imgsensor.Dgain_ratio * reg_gain) >> 10;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0221, 0x28);
	write_cmos_sensor(0x028b, 0x18);
	write_cmos_sensor(0x028c, 0x18);
	write_cmos_sensor(0x0229, 0x70);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x05a3, 0x04);
	write_cmos_sensor(0x05a4, 0x03);
	write_cmos_sensor(0x0597, 0x17);
	...
	write_cmos_sensor(0x05b1, 0x16);
	write_cmos_sensor(0x05b1, 0x3d);
	write_cmos_sensor(0x05b1, 0x10);
	write_cmos_sensor(0x05b1, 0x0d);
	write_cmos_sensor(0x05ac, 0x01);
	write_cmos_sensor(0x029f, 0xc4);
	write_cmos_sensor(0x05a0, 0xc3);
	write_cmos_sensor(0x02b0, 0x6d);
...
static void custom1_setting(void)
{
	...
	write_cmos_sensor(0x031c, 0x9f);
	write_cmos_sensor(0x0202, 0x09);
	write_cmos_sensor(0x0203, 0x00); //Exp
	write_cmos_sensor(0x0340, 0x0c);
	write_cmos_sensor(0x0341, 0x80); //FL
	write_cmos_sensor(0x0342, 0x05);
	write_cmos_sensor(0x0343, 0x20);
	write_cmos_sensor(0x0344, 0x00);
	write_cmos_sensor(0x0345, 0x06);
	...
	write_cmos_sensor(0x02b4, 0x00);
	write_cmos_sensor(0x0204, 0x04);
	write_cmos_sensor(0x0205, 0x00); //gain
	write_cmos_sensor(0x0099, 0x00);
	write_cmos_sensor(0x0351, 0x01);
	write_cmos_sensor(0x0352, 0x0e); //264+6
	write_cmos_sensor(0x0353, 0x01);
	write_cmos_sensor(0x0354, 0x68); //352+8
	write_cmos_sensor(0x034c, 0x0a);
	write_cmos_sensor(0x034d, 0x00); //2560
	write_cmos_sensor(0x034e, 0x07);
	write_cmos_sensor(0x034f, 0x80); //1920
	write_cmos_sensor(0x0108, 0x08);
	write_cmos_sensor(0x0112, 0x0a);
	write_cmos_sensor(0x0113, 0x0a);
	write_cmos_sensor(0x0114, 0x03);
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
    ...
    case SENSOR_FEATURE_GET_MODULE_SN:
        LOG_INF("gc8054F GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
        if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
            *(feature_data_32 + 1) = (gGc8054F_SN[4*(*feature_data_32) + 3] << 24)
                        | (gGc8054F_SN[4*(*feature_data_32) + 2] << 16)
                        | (gGc8054F_SN[4*(*feature_data_32) + 1] << 8)
                        | (gGc8054F_SN[4*(*feature_data_32)] & 0xFF);
        break;
    /*Henry.Chang@camera.driver 20181129, add for sensor Module SET*/
...
UINT32 GC8054F_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
