...
#include "hi846p2qmipiraw_Sensor.h"
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	if (imgsensor.autoflicker_en) {
		...
		if (realtime_fps >= 297 && realtime_fps <= 305) {
			...
			write_cmos_sensor_8(0x0006, (imgsensor.frame_length >> 8) & 0xFF);
			write_cmos_sensor_8(0x0007, imgsensor.frame_length & 0xFF);
		}
	} else {
		// Extend frame length
		write_cmos_sensor_8(0x0006, (imgsensor.frame_length >> 8) & 0xFF);
	...
	// Update Shutter
	write_cmos_sensor_8(0x0073, ((shutter & 0xFF0000) >> 16));
	write_cmos_sensor_8(0x0074, (shutter & 0x00FF00) >> 8);
	write_cmos_sensor_8(0x0075, shutter & 0x0000FF);
	LOG_INF("shutter =%d, framelength =%d", shutter,imgsensor.frame_length);
