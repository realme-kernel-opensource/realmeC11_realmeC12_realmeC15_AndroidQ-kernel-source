...
/*Tanbowen@Camera.Drv, 2020.1.1 add for register device info*/
...
static kal_uint8 gGc8054_EepData[DUALCAM_CALI_DATA_LENGTH_8ALIGN];
...
static void read_eepromData(void)
{
	...
	for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
		gGc8054_EepData[idx] = read_cmos_eeprom_8(GC8054_STEREO_START_ADDR + idx);
	}
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		gGc8054_SN[idx] = read_cmos_eeprom_8(0xB0 + idx);
		LOG_INF("gGc8054_SN[%d]: 0x%x  0x%x\n", idx, gGc8054_SN[idx]);
	}
	gGc8054_CamInfo[0] = read_cmos_eeprom_8(0x0);
	gGc8054_CamInfo[1] = read_cmos_eeprom_8(0x1);
	gGc8054_CamInfo[2] = read_cmos_eeprom_8(0x6);
	gGc8054_CamInfo[3] = read_cmos_eeprom_8(0x7);
	gGc8054_CamInfo[4] = read_cmos_eeprom_8(0x8);
	gGc8054_CamInfo[5] = read_cmos_eeprom_8(0x9);
	gGc8054_CamInfo[6] = read_cmos_eeprom_8(0xA);
	gGc8054_CamInfo[7] = read_cmos_eeprom_8(0xB);
	memcpy(&gOtpCheckdata[2][0], &gGc8054_EepData[0], 40);
	for (idx = 0; idx <40; idx++) {
		if (gOtpCheckdata[2][idx] != gOtpCheckdata[0][idx]) {
			LOG_INF("[%d]: 0x%x!=0x%x\n", idx, gOtpCheckdata[0][idx], gOtpCheckdata[2][idx]);
			return;
		}
		LOG_INF("gGc8054_SN[%d]: 0x%x\n", idx, gOtpCheckdata[2][idx]);
	}
	if (gOtpCheckdata[2][0] != 0xff
		&& gOtpCheckdata[2][1] != 0xff
		&& gOtpCheckdata[2][2] != 0xff
		&& gOtpCheckdata[2][3] != 0xff
		&& gOtpCheckdata[2][4] != 0xff
		&& gOtpCheckdata[2][5] != 0xff
		&& gOtpCheckdata[2][6] != 0xff
		&& gOtpCheckdata[2][7] != 0xff) {
		gGc8054_SN[19] = 0x5;
	}
}
			...
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r1",
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_DATA:
		...
		memcpy(&feature_para[0], gGc8054_EepData, DUALCAM_CALI_DATA_LENGTH_8ALIGN);
		break;
	/*Henry.Chang@Camera.Driver add for 18531 ModuleSN*/
