...
static kal_uint8 gImx586_SN[CAMERA_MODULE_SN_LENGTH];
static kal_uint8 gImx586_CamInfo[CAMERA_MODULE_INFO_LENGTH];
static kal_uint8 gImx586_EepData[DUALCAM_CALI_DATA_LENGTH_8ALIGN*2];
...
static void read_eepromData(void)
{
	...
	for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
		gImx586_EepData[idx] = read_cmos_eeprom_8(IMX586_STEREO_START_ADDR_WIDE + idx);
		gImx586_EepData[idx+DUALCAM_CALI_DATA_LENGTH_8ALIGN]
			= read_cmos_eeprom_8(IMX586_STEREO_START_ADDR_TELE + idx);
	}
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		gImx586_SN[idx] = read_cmos_eeprom_8(0xB0 + idx);
		LOG_INF("gImx586_SN[%d]: 0x%x  0x%x\n", idx, gImx586_SN[idx]);
	}
	gImx586_CamInfo[0] = read_cmos_eeprom_8(0x0);
	gImx586_CamInfo[1] = read_cmos_eeprom_8(0x1);
	gImx586_CamInfo[2] = read_cmos_eeprom_8(0x6);
	gImx586_CamInfo[3] = read_cmos_eeprom_8(0x7);
	gImx586_CamInfo[4] = read_cmos_eeprom_8(0x8);
	gImx586_CamInfo[5] = read_cmos_eeprom_8(0x9);
	gImx586_CamInfo[6] = read_cmos_eeprom_8(0xA);
	gImx586_CamInfo[7] = read_cmos_eeprom_8(0xB);
	memcpy(&gOtpCheckdata[0][0], &gImx586_EepData[0], 40);
	memcpy(&gOtpCheckdata[6][0],
		&gImx586_EepData[DUALCAM_CALI_DATA_LENGTH_8ALIGN], 40);
}
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	if(pStereodata != NULL) {
		pr_debug("Imx586 SET_SENSOR_OTP: 0x%x %d 0x%x %d\n",
		...
		if ((pStereodata->uSensorId == IMX586_SENSOR_ID)
				&& (data_base == IMX586_STEREO_START_ADDR || data_base == IMX586_STEREO_START_ADDR_WIDE || data_base == IMX586_STEREO_START_ADDR_TELE)
				&& (data_length == DUALCAM_CALI_DATA_LENGTH)) {
			pr_debug("Imx586 Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556],
			...
			write_eeprom_protect(1);
			msleep(6);
			pr_debug("com 0x%x:0x%x\n", data_base, read_cmos_eeprom_8(data_base));
			msleep(6);
			pr_debug("com 0x%x:0x%x\n", data_base+39, read_cmos_eeprom_8(data_base+39));
			msleep(6);
			pr_debug("innal 0x%x:0x%x\n", data_base+40, read_cmos_eeprom_8(data_base+40));
			msleep(6);
			pr_debug("innal 0x%x:0x%x\n", data_base+1556, read_cmos_eeprom_8(data_base+1556));
			msleep(6);
			pr_debug("tail1 0x%x:0x%x\n", data_base+1557, read_cmos_eeprom_8(data_base+1557));
			msleep(6);
			pr_debug("tail2 0x%x:0x%x\n", data_base+1558, read_cmos_eeprom_8(data_base+1558));
			msleep(6);
			pr_debug("tail3 0x%x:0x%x\n", data_base+1559, read_cmos_eeprom_8(data_base+1559));
			msleep(6);
			pr_debug("tail4 0x%x:0x%x\n", data_base+1560, read_cmos_eeprom_8(data_base+1560));
			msleep(6);
			pr_debug("imx586write_Module_data Write end\n");
		} else if ((pStereodata->uSensorId == IMX586_SENSOR_ID)
			&& (data_base == IMX586_AESYNC_START_ADDR)
			...
			pr_debug("readback main aesync: %x %x %x %x %x %x %x %x\n"
				, read_cmos_eeprom_8(data_base)
				, read_cmos_eeprom_8(data_base+1)
				, read_cmos_eeprom_8(data_base+2)
				, read_cmos_eeprom_8(data_base+3)
				, read_cmos_eeprom_8(data_base+4)
				, read_cmos_eeprom_8(data_base+5)
				, read_cmos_eeprom_8(data_base+6)
				, read_cmos_eeprom_8(data_base+7));
		} else {
			pr_err("Invalid Sensor id:0x%x write_imx586 eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		pr_err("Imx586write_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
...
static void read_sensor_Cali(void)
{
	...
	for (idx = 0; idx < 192; idx++) {
		...
		imx586_LRC_setting[2*idx + 1] = read_cmos_eeprom_8(addr_lrc);
	}
				     ...
				     kal_bool auto_extend_en)
{
	...
	/*wuyaokun@camera.driver,20191213,add for Tricam Long Shutter */
	int longexposure_times = 0;
	static int long_exposure_status;
			...
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8)
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_r0", DEVICE_VERSION_IMX586,
									imgsensor_info.module_id);
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	#ifdef VENDOR_EDIT
	/*Tan.Bowen@Camera.Driver 2019115 add for project 19357*/
	if (is_project(OPPO_19357) || is_project(OPPO_19358) || is_project(OPPO_19359) || is_project(OPPO_19354))
	{
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_DATA:
		...
		memcpy(&feature_para[0], gImx586_EepData, DUALCAM_CALI_DATA_LENGTH_8ALIGN*2);
		break;
	/*Henry.Chang@Camera.Driver add for google ARCode Feature verify 20190531*/
	case SENSOR_FEATURE_GET_MODULE_INFO:
		LOG_INF("imx586 GET_MODULE_CamInfo:%d %d\n", *feature_para_len, *feature_data_32);
		*(feature_data_32 + 1) = (gImx586_CamInfo[1] << 24)
					| (gImx586_CamInfo[0] << 16)
					| (gImx586_CamInfo[3] << 8)
					| (gImx586_CamInfo[2] & 0xFF);
		*(feature_data_32 + 2) = (gImx586_CamInfo[5] << 24)
					| (gImx586_CamInfo[4] << 16)
					| (gImx586_CamInfo[7] << 8)
					| (gImx586_CamInfo[6] & 0xFF);
		break;
	/*Henry.Chang@Camera.Driver add for 18531 ModuleSN*/
	case SENSOR_FEATURE_GET_MODULE_SN:
		LOG_INF("imx586 GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
		if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
			*(feature_data_32 + 1) = (gImx586_SN[4*(*feature_data_32) + 3] << 24)
						| (gImx586_SN[4*(*feature_data_32) + 2] << 16)
						| (gImx586_SN[4*(*feature_data_32) + 1] << 8)
						| (gImx586_SN[4*(*feature_data_32)] & 0xFF);
		break;
	/*Henry.Chang@camera.driver 20181129, add for sensor Module SET*/
