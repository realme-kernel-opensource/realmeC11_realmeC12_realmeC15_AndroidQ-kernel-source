...
#include "gc02m0p2qmipimono_Sensor.h"
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    LOG_INF("gc02mop2qmipimono_Sensor I2C_BUS = %d\n", I2C_BUS);
    if(I2C_BUS != 2){
        *sensor_id = 0xFFFFFFFF;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			if (deviceInfo_register_value == 0x00) {
				register_imgsensor_deviceinfo("Cam_r3",
			...
			#ifdef VENDOR_EDIT
			/* Feiping.Li@Camera.Drv, 20190716, add for pull-up avdd when main sensor is powered*/
...
static kal_uint32 open(void)
{
    ...
    #ifndef VENDOR_EDIT
    /* Ze.Pang@Camera.Drv, Modify for hal can't  detect when read sensor ID fail 20190930*/
