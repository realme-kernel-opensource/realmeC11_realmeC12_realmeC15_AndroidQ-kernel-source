...
#include "gc02m0fmipimono_Sensor.h"
...
#define PFX "GC02M0F_camera_sensor"
#define LOG_1 LOG_INF("GC02M0F,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
#define DEVICE_VERSION_GC02M0F    "gc02m0f"
...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC02M0F_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint8 gGc02m0f_SN[CAMERA_MODULE_SN_LENGTH];
static kal_uint8 gGc02m0f_CamInfo[CAMERA_MODULE_INFO_LENGTH];
static kal_uint8 gGc02m0f_EepData[DUALCAM_CALI_DATA_LENGTH_8ALIGN];
...
static void read_eepromData(void)
{
	...
	for (idx = 0; idx <DUALCAM_CALI_DATA_LENGTH; idx++) {
		gGc02m0f_EepData[idx] = read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR + idx);
	}
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		gGc02m0f_SN[idx] = read_cmos_eeprom_8(0x40 + idx);
		LOG_INF("gc02m0f_SN[%d]: 0x%x  0x%x\n", idx, gGc02m0f_SN[idx]);
	}
	gGc02m0f_CamInfo[0] = read_cmos_eeprom_8(0x0);
	gGc02m0f_CamInfo[1] = read_cmos_eeprom_8(0x1);
	gGc02m0f_CamInfo[2] = read_cmos_eeprom_8(0x6);
	gGc02m0f_CamInfo[3] = read_cmos_eeprom_8(0x7);
	gGc02m0f_CamInfo[4] = read_cmos_eeprom_8(0x8);
	gGc02m0f_CamInfo[5] = read_cmos_eeprom_8(0x9);
	gGc02m0f_CamInfo[6] = read_cmos_eeprom_8(0xA);
	gGc02m0f_CamInfo[7] = read_cmos_eeprom_8(0xB);
	memcpy(&gOtpCheckdata[5][0], &gGc02m0f_EepData[0], 40);
	for (idx = 0; idx <40; idx++) {
		if (gOtpCheckdata[5][idx] != gOtpCheckdata[1][idx]) {
			LOG_INF("[%d]: 0x%x!=0x%x\n", idx, gOtpCheckdata[1][idx], gOtpCheckdata[5][idx]);
			return;
		}
		LOG_INF("gGc02m0f[%d]: 0x%x\n", idx, gOtpCheckdata[5][idx]);
	}
	if (gOtpCheckdata[5][0] != 0xff
		&& gOtpCheckdata[5][1] != 0xff
		&& gOtpCheckdata[5][2] != 0xff
		&& gOtpCheckdata[5][3] != 0xff
		&& gOtpCheckdata[5][4] != 0xff
		&& gOtpCheckdata[5][5] != 0xff
		&& gOtpCheckdata[5][6] != 0xff
		&& gOtpCheckdata[5][7] != 0xff) {
		gGc02m0f_SN[19] = 0x22;
	}
}
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	UINT32 i = 0;
	if(pStereodata != NULL) {
		pr_debug("gc02m0f SET_SENSOR_OTP: 0x%x %d 0x%x %d\n",
		...
		if ((pStereodata->uSensorId == GC02M0F_SENSOR_ID) && (data_length == DUALCAM_CALI_DATA_LENGTH)
			&& (data_base == GC02M0F_STEREO_START_ADDR)) {
			pr_debug("gc02m0f Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556],
			...
			write_eeprom_protect(1);
			msleep(6);
			pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR));
			pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR+39));
			pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR+40));
			pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR+1556));
			pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR+1557));
			pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR+1558));
			pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR+1559));
			pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(GC02M0F_STEREO_START_ADDR+1560));
		...
		pr_err("gc02m0f write_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
    {
        ...
        LOG_INF("GC02M0FMIPI analogic gain 1x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 1.185x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 1.4x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 1.659x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 2.0x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 2.37x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 2.8x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_8<= iReg)&&(iReg < ANALOG_GAIN_9))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 3.318x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_9<= iReg)&&(iReg < ANALOG_GAIN_10))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 4.0x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_10<= iReg)&&(iReg < ANALOG_GAIN_11))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 4.74x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_11<= iReg)&&(iReg < ANALOG_GAIN_12))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 5.6x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_12<= iReg)&&(iReg < ANALOG_GAIN_13))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 6.636x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_13<= iReg)&&(iReg < ANALOG_GAIN_14))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 8.0x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_14<= iReg)&&(iReg < ANALOG_GAIN_15))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 9.48x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else if((ANALOG_GAIN_15<= iReg)&&(iReg < ANALOG_GAIN_16))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 11.2x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    else //if ((ANALOG_GAIN_16<= iReg)&&(iReg < ANALOG_GAIN_17))
    {
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 13.2725x, GC02M0FMIPI add pregain = %d\n",temp);
    }
    /*else
    {
        write_cmos_sensor(0xfe,  0x00);
        ...
        write_cmos_sensor(0xb1, (temp>>8)&0x3f);
        write_cmos_sensor(0xb2, (temp&0xff));
        LOG_INF("GC02M0FMIPI analogic gain 16x, GC02M0FMIPI add pregain = %d\n",temp);
    }*/
...
static kal_uint32 open(void)
{
    ...
    /*Don't Remove!!*/
    //gc02m0f_gcore_identify_otp();

    /* initail sequence write in  */
    sensor_init();

    /*write registers from sram*/
    //gc02m0f_gcore_update_otp();
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_EEPROM_DATA:
            ...
            memcpy(&feature_para[0], gGc02m0f_EepData, DUALCAM_CALI_DATA_LENGTH_8ALIGN);
            break;
        /*Henry.Chang@Camera.Driver add for google ARCode Feature verify 20190531*/
        case SENSOR_FEATURE_GET_MODULE_INFO:
            LOG_INF("gc02m0f GET_MODULE_CamInfo:%d %d\n", *feature_para_len, *feature_data_32);
            *(feature_data_32 + 1) = (gGc02m0f_CamInfo[1] << 24)
                        | (gGc02m0f_CamInfo[0] << 16)
                        | (gGc02m0f_CamInfo[3] << 8)
                        | (gGc02m0f_CamInfo[2] & 0xFF);
            *(feature_data_32 + 2) = (gGc02m0f_CamInfo[5] << 24)
                        | (gGc02m0f_CamInfo[4] << 16)
                        | (gGc02m0f_CamInfo[7] << 8)
                        | (gGc02m0f_CamInfo[6] & 0xFF);
            break;
        /*Henry.Chang@Camera.Driver add for 18531 ModuleSN*/
        case SENSOR_FEATURE_GET_MODULE_SN:
            LOG_INF("gc02m0f GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
            if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
                *(feature_data_32 + 1) = (gGc02m0f_SN[4*(*feature_data_32) + 3] << 24)
                            | (gGc02m0f_SN[4*(*feature_data_32) + 2] << 16)
                            | (gGc02m0f_SN[4*(*feature_data_32) + 1] << 8)
                            | (gGc02m0f_SN[4*(*feature_data_32)] & 0xFF);
            break;
        /*Henry.Chang@camera.driver 20181129, add for sensor Module SET*/
