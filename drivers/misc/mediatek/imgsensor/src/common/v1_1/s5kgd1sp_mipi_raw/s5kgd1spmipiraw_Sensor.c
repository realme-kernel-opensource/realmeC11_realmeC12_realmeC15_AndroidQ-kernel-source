...
#define DEVICE_VERSION_S5KGD1SP     "s5kgd1sp"
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	/*
	int I2C_BUS = -1;
	I2C_BUS = i2c_adapter_id(imgsensor_i2c_get_device()->pinst->pi2c_client->adapter);
	LOG_INF("s5kgd1spmipiraw_Sensor I2C_BUS = %d\n", I2C_BUS);
	if(I2C_BUS != 4){
			...
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				*sensor_id = S5KGD1SP_SENSOR_ID;
				...
				LOG_INF("s5kgd1sp_module_id=%d\n",imgsensor_info.module_id);
				if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_f", DEVICE_VERSION_S5KGD1SP, imgsensor_info.module_id);
...
static kal_uint32 s5kgd1sp_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	LOG_INF("s5kgd1sp_awb_gain: 0x100\n");
