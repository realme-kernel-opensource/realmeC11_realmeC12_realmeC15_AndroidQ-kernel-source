...
static signed int MFB_DumpReg(void)
{
	...
	log_inf("MFB Req:State:%d, procID:0x%08X, callerID:0x%08X\n",
	     g_MFB_ReqRing.MFBReq_Struct[i].State,
	     g_MFB_ReqRing.MFBReq_Struct[i].processID,
	     g_MFB_ReqRing.MFBReq_Struct[i].callerID);
	log_inf("MFB Req:enqueReqNum:%d, FrameWRIdx:%d, RrameRDIdx:%d\n",
	...
	log_inf("FrmSta[%d]:%d, FrmSta[%d]:%d, FrmSta[%d]:%d, FrmSta[%d]:%d\n",
		j,
		g_MFB_ReqRing.MFBReq_Struct[i].MfbFrameStatus[j],
		j + 1,
...
log_inf("WaitIrq Timeout:WriteReq(0x%08X), ReadReq(0x%08X), whichReq(%d)\n",
	...
	/* Store irqinfo status in here to redeuce time of spin_lock_irqsave */
		...
		log_err("MFB_IRQ_WAIT_CLEAR Error, Type(%d), WaitSta(0x%08X)",
...
#endif
#ifdef VENDOR_EDIT
/* Jianwei.Luo@Camera comment for CRDISPATCH_KEY:MfllCore/Mfb:FATAL: wait timeout of dequing from MFB blend: 1500ms CR:ALPS04953792 20200103 */
	log_inf("MFB_WAIT_IRQ Return\n");
#endif
	break;
	}
	case MFB_CLEAR_IRQ:
	...
	g_MFB_ReqRing.MFBReq_Struct[g_MFB_ReqRing.WriteIdx].FrameWRIdx,
	g_MFB_ReqRing.
		MFBReq_Struct[g_MFB_ReqRing.WriteIdx].enqueReqNum);
	}
		spin_unlock_irqrestore(
			&(MFBInfo.
			SpinLockIrq[MFB_IRQ_TYPE_INT_MFB_ST]),
...
static signed int __init MFB_Init(void)
{
	...
	proc_entry = proc_create("mfb_dump",
		0444,
		isp_mfb_dir,
		&mfb_dump_proc_fops);

	proc_entry = proc_create("mfb_reg",
		0644,
		isp_mfb_dir,
		&mfb_reg_proc_fops);
