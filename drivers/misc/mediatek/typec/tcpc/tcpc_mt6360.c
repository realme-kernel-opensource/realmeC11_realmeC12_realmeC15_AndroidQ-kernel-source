...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/01/10, sjc Add for WD */
...
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/04/26, sjc Add for WD info */
#define INVALID_SBU_VOLT	-1
static int typec_sbu_volt_mv = INVALID_SBU_VOLT;
int oppo_get_typec_sbu_voltage(void)
{
	return typec_sbu_volt_mv;
}
EXPORT_SYMBOL(oppo_get_typec_sbu_voltage);
...
void oppo_set_water_detect(bool enable)
{
	if (enable) {
		if (oppo_mt6360_chip)
...
EXPORT_SYMBOL(oppo_set_water_detect);

int oppo_get_water_detect(void)
{
	if (water_detect_feature)
		return 1;
	return 0;
}
EXPORT_SYMBOL(oppo_get_water_detect);
#endif /*VENDOR_EDIT*/
...
static int mt6360_regmap_init(struct mt6360_chip *chip)
{
	...
	if (!chip->m_dev) {
		dev_notice(chip->dev, "%s register fail\n", __func__);
		return -EINVAL;
	}
	return 0;
}
...
/* Jianchao.Shi@BSP.CHG.Basic, 2019/04/08, sjc Add for WD (C to DP/HDMI) */
	...
	if (en) {
		...
		if (ret < 0) {
			dev_notice(chip->dev, "%s usbid src on 100ms fail\n",
					__func__);
			return ret;
		}
		...
		if (ret < 0) {
			dev_notice(chip->dev, "%s usbid rup75k fail\n",
				__func__);
			return ret;
		}
	}
...
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/01/10, sjc Modify for WD */
	...
	if (ret <= 0) {
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/01/16, sjc Add for WD */
...
static int mt6360_pmu_tcpc_irq_register(struct tcpc_device *tcpc)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6360_pmu_tcpc_irq_desc); i++) {
		...
		ret = devm_request_threaded_irq(chip->dev, irq_desc->irq, NULL,
						...
						chip);
		if (ret < 0)
			dev_notice(chip->dev, "%s request %s irq fail\n",
				__func__, irq_desc->name);
	}
	return ret;
}
...
static int mt6360_init_alert(struct tcpc_device *tcpc)
{
...
#endif /* DEBUG_GPIO */
	if (ret < 0) {
		dev_notice(chip->dev, "%s fail to request GPIO%d(%d)\n",
	...
	if (ret < 0) {
		dev_notice(chip->dev, "%s fail to set GPIO%d as input pin(%d)\n",
			__func__, chip->irq_gpio, ret);
		goto init_alert_err;
	}
	...
	if (chip->irq <= 0) {
		dev_notice(chip->dev, "%s gpio to irq fail, chip->irq(%d)\n",
			__func__, chip->irq);
		goto init_alert_err;
	}
	...
	if (IS_ERR(chip->irq_worker_task)) {
		dev_notice(chip->dev, "%s could not create tcpc task\n",
			__func__);
		goto init_alert_err;
	}
	...
	if (ret < 0) {
		dev_notice(chip->dev, "%s fail to request irq%d, gpio%d (%d)\n",
...
static inline int mt6360_init_water_detection(struct tcpc_device *tcpc)
{
	 ...
	 */
#ifndef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/02/14, sjc Modify for WD */
...
static inline bool mt6360_is_audio_device(struct tcpc_device *tcpc, int usbid)
{
	...
	if (ret < 0) {
		dev_notice(chip->dev, "%s usbid rup1k fail\n", __func__);
		goto not_auddev;
	}
			   ...
			   struct mt6360_tcpc_platform_data *pdata)
{
	...
	if (!np) {
		dev_notice(dev, "%s find node fail\n", __func__);
		return -ENODEV;
	}
	...
	if (ret < 0) {
		dev_notice(dev, "%s no intr_gpio info(gpiolib)\n", __func__);
		return ret;
	}
	chip->irq_gpio = ret;
#else
	ret = of_property_read_u32(np, "mt6360pd,intr_gpio_num",
				   &chip->irq_gpio);
	if (ret < 0) {
		dev_notice(dev, "%s no intr_gpio info\n", __func__);
		return ret;
	}
...
static int mt6360_tcpcdev_init(struct mt6360_chip *chip, struct device *dev)
{
	...
	np = of_find_node_by_name(NULL, "type_c_port0");
	if (!np) {
		dev_notice(dev, "%s find type_c_port0 fail\n", __func__);
		return -ENODEV;
	}
...
static inline int mt6360_check_revision(struct i2c_client *client)
{
	...
	if (ret < 0) {
		dev_notice(&client->dev, "%s read chip ID fail\n", __func__);
		return ret;
	}
	if (id != MEDIATEK_6360_VID) {
		dev_notice(&client->dev, "%s check vid fail(0x%04X)\n",
			__func__, id);
		return -ENODEV;
	}
	...
	if (ret < 0) {
		dev_notice(&client->dev, "%s read product ID fail\n", __func__);
		return ret;
	}

	if (id != MEDIATEK_6360_PID) {
		dev_notice(&client->dev, "%s check pid fail(0x%04X)\n",
			__func__, id);
		return -ENODEV;
	}
	...
	if (ret < 0) {
		dev_notice(&client->dev, "%s read device ID fail\n", __func__);
		return ret;
	}
...
/* Jianchao.Shi@BSP.CHG.Basic, 2019/04/08, sjc Add for WD (C to DP/HDMI) */
	...
	if (ret < 0) {
		dev_notice(chip->dev, "%s regmap init fail(%d)\n",
			__func__, ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		dev_notice(chip->dev, "%s tcpc dev init fail\n", __func__);
		goto err_tcpc_reg;
	}
	...
	if (ret < 0) {
		dev_notice(chip->dev, "%s init ctd fail\n", __func__);
		goto err_tcpc_reg;
	}

	ret = mt6360_software_reset(chip->tcpc);
	if (ret < 0) {
		dev_notice(chip->dev, "%s sw reset fail\n", __func__);
		goto err_sw_reset;
	}

	ret = mt6360_init_alert(chip->tcpc);
	if (ret < 0) {
		dev_notice(chip->dev, "%s init alert fail\n", __func__);
		goto err_sw_reset;
	}
