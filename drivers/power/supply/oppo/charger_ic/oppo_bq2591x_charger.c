...
static int oppo_bq2591x_set_input_current_limit(int curr)
{
	u8 val;
...
int oppo_bq2591x_set_aicr(int current_ma)
{
	...
	dev_info(g_bq->dev, "%s:%d\n", __func__,current_ma);
     ...
     if (strcmp(g_bq->chg_dev_name, "secondary_chg") == 0) {
		...
		return oppo_bq2591x_set_input_current_limit(current_ma);
	}
	...
	oppo_bq2591x_set_input_current_limit(4200);
	...
	i = 1; /* 500 */
	oppo_bq2591x_set_input_current_limit(usb_icl[i]);
	msleep(90);
...
int oppo_bq2591x_set_cv(int mv)
{
	u8 val;
...
struct oppo_chg_operations  oppo_chg_bq2591x_ops = {
	...
	.otg_enable = oppo_bq2591x_enable_otg,
	.otg_disable = oppo_bq2591x_disable_otg,
