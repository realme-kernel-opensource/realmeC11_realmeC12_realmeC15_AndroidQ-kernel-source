...
struct bq2589x {
	...
	enum power_supply_type oppo_chg_type;
...
static int bq2589x_get_charger_type(struct bq2589x *bq, enum charger_type *type)
{
	...
	case BQ2589X_VBUS_TYPE_NONE:
		...
		bq->oppo_chg_type = POWER_SUPPLY_TYPE_UNKNOWN;
		break;
	case BQ2589X_VBUS_TYPE_SDP:
		chg_type = STANDARD_HOST;
		bq->oppo_chg_type = POWER_SUPPLY_TYPE_USB;
		break;
	case BQ2589X_VBUS_TYPE_CDP:
		chg_type = CHARGING_HOST;
		bq->oppo_chg_type = POWER_SUPPLY_TYPE_USB_CDP;
		break;
	case BQ2589X_VBUS_TYPE_DCP:
		chg_type = STANDARD_CHARGER;
		bq->oppo_chg_type = POWER_SUPPLY_TYPE_USB_DCP;
		break;
...
static void bq2589x_plug_in_setting(struct bq2589x *bq)
{
	if(bq->otg_enable)
		return;
...
static void bq2589x_plug_out_setting(struct bq2589x *bq)
{
	...
	bq2589x_switch_to_hvdcp(bq, HVDCP_DPF_DMF);
...
static irqreturn_t bq2589x_irq_handler(int irq, void *data)
{
	...
	if (!ret && prev_chg_type != bq->chg_type && bq->chg_det_enable) {
		Charger_Detect_Release();//CHECK
		if (bq->chg_type == CHARGER_UNKNOWN) {
			bq2589x_plug_out_setting(bq);
...
static void chrdet_int_handler(void)
{
    if (pmic_get_register_value(PMIC_RGS_CHRDET)) {
		pr_notice("charger type: charger IN\n");
		bq2589x_plug_in_setting(g_bq);
    } else {
		pr_notice("charger type: charger OUT\n");
		bq2589x_plug_out_setting(g_bq);
	}
}
...
void oppo_bq2589x_dump_registers(void)
{
	bq2589x_dump_regs(g_bq);
}

int oppo_bq2589x_kick_wdt(void)
{
	return bq2589x_reset_watchdog_timer(g_bq);
}

int oppo_bq2589x_set_ichg(int cur)
{
	struct oppo_chg_chip *chip = g_oppo_chip;
	u32 uA = cur*1000;
	u32 temp_uA;
...
void oppo_bq2589x_set_mivr(int vbatt)
{
...
int oppo_bq2589x_set_aicr(int current_ma)
{
	struct oppo_chg_chip *chip = g_oppo_chip;
	int rc = 0, i = 0;
	int chg_vol = 0;
...
int oppo_bq2589x_set_input_current_limit(int current_ma)
{
...
int oppo_bq2589x_set_cv(int cur)
{
	return bq2589x_set_chargevolt(g_bq, cur);
}

int oppo_bq2589x_set_ieoc(int cur)
{
	return bq2589x_set_term_current(g_bq, cur);
}

int oppo_bq2589x_charging_enable(void)
{
	return bq2589x_enable_charger(g_bq); 
}

int oppo_bq2589x_charging_disable(void)
{
...
int oppo_bq2589x_hardware_init(void)
{
	int ret = 0;
...
int oppo_bq2589x_is_charging_enabled(void)
{
	
	return g_bq->charge_enabled;
}

int oppo_bq2589x_is_charging_done(void)
{
	int ret = 0;
	bool done;
...
int oppo_bq2589x_enable_otg(void)
{
	int ret = 0;
...
int oppo_bq2589x_disable_otg(void)
{
	int ret = 0;
...
int oppo_bq2589x_disable_te(void)
{
	return  bq2589x_enable_term(g_bq, false);
}

int oppo_bq2589x_get_chg_current_step(void)
{
	return BQ2589X_ICHG_LSB;
}

int oppo_bq2589x_get_charger_type(void)
{
	return g_bq->oppo_chg_type;
}

int oppo_bq2589x_charger_suspend(void)
{
	return 0;
}

int oppo_bq2589x_charger_unsuspend(void)
{
	return 0;
}

int oppo_bq2589x_set_rechg_vol(int vol)
{
	return 0;
}

int oppo_bq2589x_reset_charger(void)
{
	return 0;
}

bool oppo_bq2589x_check_charger_resume(void)
{
	return true;
}

void oppo_bq2589x_set_chargerid_switch_val(int value)
{
	return;
}

int oppo_bq2589x_get_chargerid_switch_val(void)
{
	return 0;
}

int oppo_bq2589x_get_charger_subtype(void)
{
	struct charger_manager *info = NULL;
...
bool oppo_bq2589x_need_to_check_ibatt(void)
{
	return false;
}

int oppo_bq2589x_get_dyna_aicl_result(void)
{
	int mA = 0;
...
int oppo_bq2589x_set_qc_config(void)
{
	struct oppo_chg_chip *chip = g_oppo_chip;
	struct charger_manager *info = NULL;
	...
	if(g_oppo_chip->camera_on) {
		dev_info(g_bq->dev, "%s:camera disable_hight_vbus\n", __func__);
		return false;
	}
...
int oppo_bq2589x_chg_pd_setup(void)
{
	return 0;
}

int oppo_bq2589x_enable_qc_detect(void)
{
	return 0;
}

bool oppo_bq2589x_get_shortc_hw_gpio_status(void)
{
	return false;
}

int oppo_bq2589x_chg_set_high_vbus(bool en)
{
	int subtype;
	struct oppo_chg_chip *chip = g_oppo_chip;
	...
	if(en){
		...
		if (g_oppo_chip->camera_on) {
			dev_info(g_bq->dev, "%s:camera already setup5v no need 9v\n", __func__);
			return false;
		}
	}else{
	...
	subtype=oppo_bq2589x_get_charger_subtype();
...
struct oppo_chg_operations  oppo_chg_bq2589x_ops = {
	.dump_registers = oppo_bq2589x_dump_registers,
	.kick_wdt = oppo_bq2589x_kick_wdt,
	.hardware_init = oppo_bq2589x_hardware_init,
	.charging_current_write_fast = oppo_bq2589x_set_ichg,
	.set_aicl_point = oppo_bq2589x_set_mivr,
	.input_current_write = oppo_bq2589x_set_input_current_limit,
	.float_voltage_write = oppo_bq2589x_set_cv,
	.term_current_set = oppo_bq2589x_set_ieoc,
	.charging_enable = oppo_bq2589x_charging_enable,
	.charging_disable = oppo_bq2589x_charging_disable,
	.get_charging_enable = oppo_bq2589x_is_charging_enabled,
	.charger_suspend = oppo_bq2589x_charger_suspend,
	.charger_unsuspend = oppo_bq2589x_charger_unsuspend,
	.set_rechg_vol = oppo_bq2589x_set_rechg_vol,
	.reset_charger = oppo_bq2589x_reset_charger,
	.read_full = oppo_bq2589x_is_charging_done,
	.otg_enable = oppo_bq2589x_enable_otg,
	.otg_disable = oppo_bq2589x_disable_otg,
	.set_charging_term_disable = oppo_bq2589x_disable_te,
	.check_charger_resume = oppo_bq2589x_check_charger_resume,

	.get_charger_type = oppo_bq2589x_get_charger_type,
	.get_charger_volt = battery_get_vbus,
//	int (*get_charger_current)(void);
	.get_chargerid_volt = NULL,
    .set_chargerid_switch_val = oppo_bq2589x_set_chargerid_switch_val,
    .get_chargerid_switch_val = oppo_bq2589x_get_chargerid_switch_val,
    ...
    .get_chg_current_step = oppo_bq2589x_get_chg_current_step,
    .need_to_check_ibatt = oppo_bq2589x_need_to_check_ibatt,
    .get_dyna_aicl_result = oppo_bq2589x_get_dyna_aicl_result,
    .get_shortc_hw_gpio_status = oppo_bq2589x_get_shortc_hw_gpio_status,
//	void (*check_is_iindpm_mode) (void);
    .oppo_chg_get_pd_type = NULL,
    .oppo_chg_pd_setup = oppo_bq2589x_chg_pd_setup,
	.get_charger_subtype = oppo_bq2589x_get_charger_subtype,
	.set_qc_config = oppo_bq2589x_set_qc_config,
	.enable_qc_detect = oppo_bq2589x_enable_qc_detect,
	.oppo_chg_get_pe20_type = NULL,
	.oppo_chg_pe20_setup = NULL,
	.oppo_chg_reset_pe20 = NULL,
	.oppo_chg_set_high_vbus = oppo_bq2589x_chg_set_high_vbus,
	.enable_shipmode = bq2589x_enable_shipmode,
};

static void aicr_setting_work_callback(struct work_struct *work)
{
	oppo_bq2589x_set_aicr(g_bq->aicr);
}
