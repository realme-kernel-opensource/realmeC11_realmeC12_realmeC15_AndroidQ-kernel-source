...
oppo_batt_data oppo_battery_id_vol[OPPO_MAX_NUM] = {[0]={550000,790000},[1]={1040000,1325000}};
int battery_id_vol = 0;
int bat_id_num = 0;
extern void devinfo_info_set(char *name, char *version, char *manufacture);

static bool is_PASCAL = 0;
static bool is_PASCALE = 0;
static int __init battery_board_id_get(char *str)
{
	char hardware_info[20];
	
    strncpy(hardware_info, str,strlen(str)+1);
    pr_info("%s BoardID= %s\n ",__func__,hardware_info);
	
	if(strncmp(hardware_info,"S98635",6) == 0){
		is_PASCAL = 1;
	}else if(strncmp(hardware_info,"S98639",6) == 0){
		is_PASCALE = 1;
	}else{
		is_PASCAL = 0;
		is_PASCALE = 0;
...
__setup("board_id=", battery_board_id_get);

bool is_project_pascal(void)
{
	return is_PASCAL;
}

/*Sidong.Zhao@WT.CHG,2020/6/28,add for oppoversion*/
bool is_project_pascalE(void)
{
	return is_PASCALE;
}
#endif /*ODM_WT_EDIT*/
...
/*Sidong.Zhao@ODM_WT.BSP.CHG 2019/11/11,distinguish battery id*/
	...
	if(is_project_pascal() || is_project_pascalE()){
		offset = OPPO_MAX_NUM;
	}
	
	if((bat_id_num >= OPPO_ATL)&&(bat_id_num < OPPO_MAX_NUM)){
		gm.battery_id = bat_id_num + offset;
	}else{
		gm.battery_id = 0 + offset;
	}
	
	if(bat_id_num == OPPO_ATL){
		sprintf(battery_name,"oppo-atl-4v45-%smah",(is_project_pascal()||is_project_pascalE())?"6000":"5000");
		hardwareinfo_set_prop(HARDWARE_BATTERY_ID,battery_name);
		devinfo_info_set("battery","4.45v","ATL");
	}else if(bat_id_num == OPPO_SDI){
		sprintf(battery_name,"oppo-sdi-4v45-%smah",(is_project_pascal()||is_project_pascalE())?"6000":"5000");
		hardwareinfo_set_prop(HARDWARE_BATTERY_ID,battery_name);
		devinfo_info_set("battery","4.45v","SDI");
	}else{
...
#ifdef ODM_WT_EDIT
/*Junbo.Guo@ODM_WT.BSP.CHG 2020/04/27,battery type detection*/
...
#ifdef ODM_WT_EDIT
/* Sidong.Zhao@WT.CHG, 2020/6/15, wake up oppo chg work after soc initial done */
...
#ifdef ODM_WT_EDIT
	//Sidong.Zhao@WT.CHG,2020/6/15,wake up oppo chg work after soc initial done
		if (!gm.soc_initial_done) {
			gm.soc_initial_done = true;
			oppo_chg_wake_update_work();
			bm_err("soc_initial_done\n");
		}
#endif
	}
	break;
