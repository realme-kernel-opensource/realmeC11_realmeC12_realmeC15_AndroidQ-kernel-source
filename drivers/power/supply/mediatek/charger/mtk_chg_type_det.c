...
extern bool g_oppo_charge_init_done;
#endif /*ODM_WT_EDIT*/
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	struct chg_type_info *cti;
#ifdef VENDOR_EDIT
/* Jianchao.Shi@BSP.CHG.Basic, 2019/03/28, sjc Add for charging */
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/5/14, kernel panic for NULL pointer*/
	if (battery_psy && g_oppo_charge_init_done)
#else
		if (battery_psy)
#endif /*ODM_WT_EDIT*/
...
#else
			((mtk_chg->chg_type == NONSTANDARD_CHARGER) && oppo_get_chg_unwakelock() == 0))
#endif
			mt_usb_connect();
		else
			mt_usb_disconnect();
		...
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			if (cti->tcpc_kpoc) {
				...
				pr_info("%s KPOC Plug out, vbus = %d\n",
					__func__, vbus);
#ifndef VENDOR_EDIT
/*LIZHIJIE@BSP.CHG.Basic lzj delete for shutdown charger*/
...
#ifndef VENDOR_EDIT
/* Jianwei.Ye@BSP.CHG.Basic, 2018/11/09, sjc Delete for charging */
		if (cti->chg_consumer)
