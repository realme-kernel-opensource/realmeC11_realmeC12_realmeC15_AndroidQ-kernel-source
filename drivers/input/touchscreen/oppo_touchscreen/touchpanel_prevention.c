...
void init_kernel_grip_proc(struct proc_dir_entry *prEntry_tp, struct kernel_grip_info *grip_info)
{
    ...
    prEntry_tmp = proc_create_data("oppo_tp_direction", 0666, prEntry_tp, &touch_dir_proc_fops, grip_info);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create oppo_tp_direction proc entry, %d\n", __func__, __LINE__);
    }
}
...
struct kernel_grip_info *kernel_grip_init(struct device *dev)
{
    ...
    int dead_width[2] = {0}, makeup_para[10] = {0}, large_para[3] = {0}, cond_para[2] = {0}, no_handle_para[3] = {0};
    int temp_array[2] = {0}, cond_width[4] = {0}, large_width[2] = {0}, large_corner_width[3] = {0}, eli_width[4] = {0};
    struct grip_zone_area *grip_zone = NULL;
    ...
    if (grip_zone) {
        grip_zone->start_x = 0;
        grip_zone->start_y = grip_info->max_y - eli_width[2];
        grip_zone->x_width = eli_width[3];
        grip_zone->y_width = eli_width[2];
    ...
    grip_zone = kzalloc(sizeof(struct grip_zone_area), GFP_KERNEL);
    if (grip_zone) {
        grip_zone->start_x = grip_info->max_x - eli_width[3];
        grip_zone->start_y = 0;
        grip_zone->x_width = eli_width[3];
        grip_zone->y_width = eli_width[2];
