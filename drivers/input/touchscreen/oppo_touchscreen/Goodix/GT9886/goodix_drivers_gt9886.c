 ...
 * Zengpeng.Chen@Bsp.Group.Tp add first for basic function ,fw debug, i2c doze flow
 * Qicai.Gu@Bsp.Group.Tp add auto test ,fw update,irq flow,fp feature,edge mode
 * Wenjie.Zhong@Bsp.Group.Tp add fw cfg bin,fp feature,gesture feature,game mode,edge mode
 ****************************************************************/
...
static u32 goodix_u32_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
    ...
    if ( (chip_info->touch_data[0] & GOODIX_TOUCH_EVENT) == GOODIX_TOUCH_EVENT ) {
        ...
        if (check_sum || all_zero) {
            TPD_INFO("irq checksum is invalid at else\n");
            goto IGNORE_CLEAR_IRQ;
        }
    }
