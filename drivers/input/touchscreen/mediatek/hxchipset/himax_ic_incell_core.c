...
static void himax_mcu_read_FW_ver(void)
{
	...
	if (hx_ctpmodule == 0) {
		sprintf(Ctp_name,"XL,HX83102,FW:%X\n",ic_data->vendor_touch_cfg_ver);
		sprintf(himax_version,"XL_HX83102_%0X",ic_data->vendor_touch_cfg_ver);
		devinfo_info_tp_set(himax_version, "XL",BOOT_UPGRADE_FWNAME_SIGNED);
	} else if (hx_ctpmodule == 1) {
		sprintf(Ctp_name,"HLT_B8,HX83102,FW:%X\n",ic_data->vendor_touch_cfg_ver);
		sprintf(himax_version,"HLT_B8_HX83102_%0X",ic_data->vendor_touch_cfg_ver);
		devinfo_info_tp_set(himax_version, "HLT_B8",BOOT_UPGRADE_FWNAME_HLT_B8_SIGNED);
	} else {
		I("Not fund touchscreen!\n");
	}
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	if (private_ts->g_fw_sta) {
		...
		request_fw_headfile->size =private_ts->firmware_headfile.size;
		request_fw_headfile->data = private_ts->firmware_headfile.data;
		tmp_fw_entry = request_fw_headfile;
	...
	ret = request_firmware(&tmp_fw_entry, hx_fw->firmware_name,
		private_ts->dev);
	if (ret < 0) {
#if defined(__EMBEDDED_FW__)
		fw_entry = &g_embedded_fw;
...
void himax_mcu_0f_operation(struct work_struct *work)
{
	...
	err = request_firmware(&fw_entry, hx_fw->firmware_name,
		private_ts->dev);
	if (err < 0) {
#if defined(__EMBEDDED_FW__)
		fw_entry = &g_embedded_fw;
...
static bool himax_mcu_no_jiter_en(uint8_t no_jiter_en)
{
	...
	do {
		if (no_jiter_en) {
			...
			g_core_fp.fp_register_write(pfw_op->addr_fw_no_jiter_en, DATA_LEN_4, tmp_data, 0);
		...
		g_core_fp.fp_register_read(pfw_op->addr_fw_no_jiter_en, DATA_LEN_4, tmp_data, 0);
...
static bool himax_mcu_edge_limit_en(uint8_t edge_limit_en)
{
	...
	do {
		if (edge_limit_en) {
			himax_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_fw_edge_limit,DATA_LEN_4, tmp_data, 0);
			himax_parse_assign_cmd(fw_func_handshaking_pwd, back_data, 4);
		} else {
			himax_parse_assign_cmd(fw_data_clear, tmp_data, 4);
			g_core_fp.fp_register_write(pfw_op->addr_fw_edge_limit,DATA_LEN_4, tmp_data, 0);
