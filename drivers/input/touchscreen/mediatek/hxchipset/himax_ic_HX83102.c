...
static int hx83102d_0f_overlay(int ovl_type, int mode)
{
	...
	if ((*kp_private_ts)->g_fw_sta) {
		I("get from g_fw_buf\n");
		request_fw_headfile->size = (*kp_private_ts)->g_fw_len;
		request_fw_headfile->data = (*kp_private_ts)->g_fw_buf;
		fwp = request_fw_headfile;
		(*kp_private_ts)->using_headfile = false;
		use_bin_file= false;
	} else if((*kp_private_ts)->firmware_headfile.data != NULL){
		I("request firmware failed, get from headfile\n");
		request_fw_headfile->size =(*kp_private_ts)->firmware_headfile.size;
		request_fw_headfile->data = (*kp_private_ts)->firmware_headfile.data;
		fwp = request_fw_headfile;
		(*kp_private_ts)->using_headfile = true;
		use_bin_file= false;
	} else {
		I("firmware buffer and i_file is NULL! request for bin!\n");
		ret = request_firmware(&fwp, hx_fw->firmware_name,(*kp_private_ts)->dev);
		if (ret < 0) {
...
#else
		I("firmware buffer and i_file is NULL! request for bin!\n");
		ret = request_firmware(&fwp, hx_fw->firmware_name,
		(*kp_private_ts)->dev);
		if (ret < 0) {
			E("%s: request firmware FAIL!!!\n", __func__);
			return ret;
	...
	count = 0;
#if defined(OPPO_PROC_NODE)
	kp_himax_int_enable(0);
	kp_g_core_fp->fp_system_reset();
	hx83102d_sense_off(true);
	mutex_lock(&(*kp_private_ts)->fw_update_lock);
#else
	do {
...
#if defined(OPPO_PROC_NODE)
		mutex_unlock(&(*kp_private_ts)->fw_update_lock);
		kp_g_core_fp->fp_system_reset();
		himax_hx83102d_reload_to_active();
	...
	if ( use_bin_file == true ){
		release_firmware(fwp);
	}

	/* rescue mechanism */
	if (count >= 10) {
		kp_g_core_fp->fp_0f_op_file_dirly(hx_fw->firmware_name);
