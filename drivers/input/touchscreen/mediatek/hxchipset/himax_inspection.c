...
int himax_self_test_data_init(void)
{
	...
	char *file_name_2 = hx_fw->openshort_test_threshold;
	int ret = HX_INSP_OK;
	int err = 0;
	int i = 0;
...
static int himax_chip_self_test(struct seq_file *s, void *v)
{
	...
	g_core_fp.fp_0f_op_file_dirly(hx_fw->firmware_mp_name);
	...
	//seq_printf(s, "%d error(s)\n", err_count);
	if(err_count == 0){
		seq_printf(s, "%d errors. All test passed.\n", err_count);
	}else{
		seq_printf(s, "MP test fail.\n");
	}
	err_count = 0;
...
int himax_black_chip_self_test(struct seq_file *s, void *v)
{
	...
	I("%s:Black IN: i=%d,in_self_test = %d\n", __func__,i,private_ts->in_self_test);
