			...
			unsigned long event, void *data)
{
	...
	evdata = data;
#ifndef ODM_WT_EDIT
//#Yize.Zhang@ODM_WT.BSP.Touchscreen.Base,2020/04/09,bringup remove for tddi(no flash) touchscreen resume  and suspend  control flow.
...
#ifdef ODM_WT_EDIT
//#Yize.Zhang@ODM_WT.BSP.Touchscreen.Base,2020/04/09,bringup add for tddi(no flash) touchscreen resume  and suspend  control flow.
...
/*for some projects, we do not use mtk framebuffer*/
	...
	if (1600 == TPD_RES_Y)
#ifndef ODM_WT_EDIT
//#Yize.Zhang@ODM_WT.BSP.Touchscreen.Base,2020/04/07,bringup  modify for tddi(no flash) touchscreen for 1600 resolution.
		TPD_RES_Y = 1536;
#else
		TPD_RES_Y = 1600;
#endif
	...
	if (touch_type == 1) {
		...
		input_set_abs_params(tpd->dev,
			ABS_MT_POSITION_X, 0, TPD_RES_X - 1, 0, 0);
		input_set_abs_params(tpd->dev,
			ABS_MT_POSITION_Y, 0, TPD_RES_Y - 1, 0, 0);
#if defined(CONFIG_MTK_S3320) || defined(CONFIG_MTK_S3320_47) \
	|| defined(CONFIG_MTK_S3320_50) || defined(CONFIG_MTK_MIT200) \
	|| defined(CONFIG_TOUCHSCREEN_SYNAPTICS_S3528) \
	|| defined(CONFIG_MTK_S7020) \
