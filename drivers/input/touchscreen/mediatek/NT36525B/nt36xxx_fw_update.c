		...
		uint32_t SRAM_addr, uint32_t size, uint32_t BIN_addr)
{
	...
	for (i = 0 ; i < count ; i++) {
		...
		ret = NVT_CTP_SPI_WRITE(ts->client, fwbuf, len+1);
		if (ret) {
...
static int32_t nvt_check_fw_checksum(void)
{
	...
	ret = NVT_CTP_SPI_READ(ts->client, fwbuf, len+1);
	if (ret) {
		...
		uint32_t G_CHECKSUM_ADDR, uint32_t crc)
{
	...
	NVT_CTP_SPI_WRITE(ts->client, fwbuf, 4);
	...
	fwbuf[1] = (size) & 0xFF;
	fwbuf[2] = (size >> 8) & 0xFF;
	NVT_CTP_SPI_WRITE(ts->client, fwbuf, 3);
	...
	fwbuf[3] = (crc >> 16) & 0xFF;
	fwbuf[4] = (crc >> 24) & 0xFF;
	NVT_CTP_SPI_WRITE(ts->client, fwbuf, 5);
