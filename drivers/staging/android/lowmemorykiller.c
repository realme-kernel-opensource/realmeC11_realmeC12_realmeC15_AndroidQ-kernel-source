...
static unsigned int almk_swap_ratio1 = 4;
...
#ifdef VENDOR_EDIT
/*huacai.zhou@PSW.BSP.Kernel.MM, 2018/03/12, use aggressive for lowmem*/
	if (totalram_pages <= TOTALRAM_3GB) {
...
/*tianwen@PSW.BSP.Memory, 2019-04-26, check free and file pages before almk*/
		if (to_be_aggressive > 0 && i >= 0) {
			if (((other_free + other_file) < totalram_pages/almk_totalram_ratio)&& totalram_pages >= 3*SZ_1G_PAGES)
			{
			...
			{
			 i -= *to_be_aggressive;
			if (other_file < totalram_pages/6)
				amr_adj = lowmem_adj[i];
			}
		}
#else
